...
for(const t of Object.values(TWIN_FORMS)){
 ...
 if(first){assert.ok(['gravitymirror','chainburst','blastlance','frostkaleidoscope','lightningpetal','returnflare'].includes(first.id),`${t.id} unexpected duplicate path`);assert.notEqual(first.desc,t.desc,'a direct fusion and a twin awakening must fight differently');}
}
