...
for(const form of candidates){
 ...
 let damage=0,vfxCount=0,finalHits=0;const sounds=[];
 ...
 const combat=createFormCombat(scene,{player,enemies:()=>enemies,hit:(_e,amount,metadata)=>{assert.ok(Number.isFinite(amount)&&amount>=0,form.id);if(metadata?.kind)assert.ok(ALL_FORMS[metadata.kind],`${form.id}: invalid hit kind ${metadata.kind}`);if(metadata?.phase==='final')finalHits++;damage+=amount;return true;},
  blocked:()=>false,boundary:()=>false,constrain:p=>p,vfx,sound:id=>sounds.push(id)});
 combat.set(form.id,5,{openingDelay:1});
 assert.equal(combat.audioEvent(),finalProjectileStyle(form.id).audio,`${form.id}: visible shot and firing sound disagree`);
 ...
 assert.ok(sounds.length>0&&sounds.every(id=>AUDIO_EVENTS[id]),`${form.id}: silent or unknown impact cue`);
...
console.log(`Final branch combat: ${candidates.length} new branches fire, surge and clear; max queued ${maxShots} shots/${maxEvents} events/${maxFields} fields; no pattern damage ${zeroFinal.join(',')||'none'}.`);
