...
assert.equal(new Set(Object.values(SIGNATURES).map(s=>s.name)).size,19,'signature names are unique');
...
assert.equal(plain.hits,1);assert.equal(plain.arcs,0);assert.equal(plain.echoes,0);assert.equal(plain.pull,0);assert.equal(plain.slow,0);
assert.ok(full.radius>plain.radius&&full.hits===2&&full.arcs===ACTIVE.maxArcs&&full.pull>0&&full.slow>0&&full.bossScale>1);
