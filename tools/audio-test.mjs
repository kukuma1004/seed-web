...
for(const id of ['garden','combat','boss']){const music=MUSIC_SCENES[id];assert.ok(music.root>0&&music.gain>0&&music.step>0);assert.equal(music.ratios.length,2);assert.ok(music.bell.length>=3);assert.ok(music.chords.length>=4&&music.chords.every(chord=>chord.length===2));}
