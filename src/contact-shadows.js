...
const RADII=Object.freeze({player:.46,swarm:.3,hound:.52,caster:.56,shield:.72,turret:.68,warden:1.18,austin:1.35});
...
export function createContactShadows(scene,capacity=CONTACT_SHADOW_CAPACITY){
 ...
 const add=(actor,type)=>{
  ...
  const radius=contactShadowRadius(type||actor?.type);position.set(group.position.x,.045,group.position.z);scale.set(radius,radius*.56,1);matrix.compose(position,quaternion,scale);mesh.setMatrixAt(mesh.count++,matrix);
 };
