 ...
 async function historicalAustinWins(goal=10){
  ...
  for(const season of [SEASON,PREVIOUS_SEASON,ARCHIVE_SEASON]){
   ...
   for(let at=0;at<records.length&&wins<goal;at+=8){
    const batch=records.slice(at,at+8);
    const results=await Promise.allSettled(batch.map(([id])=>recent[id]?Promise.resolve(recent[id]):read(`${config.databaseURL}/${paths.builds}/${encodeURIComponent(id)}.json?auth=${encodeURIComponent(s.idToken)}`)));
    results.forEach((result,index)=>{const [id,run]=batch[index],build=result.status==='fulfilled'?result.value:null;if(validBuild(build)&&build.uid===s.uid)wins+=Math.min(build.austins,Math.floor((run.cycle+1)/5));});
   }
   if(wins>=goal)break;
  }
  return wins;
