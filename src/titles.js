...
export function titleState({austin=false,alwaysBeginner=false,discovered=0,total=Infinity,badges=[],equipped=''}={}){
 ...
 if(codex)titles.push({id:'codex',name:CODEX_TITLE,perk:codexSpeed>0?`도감 ${n}개 · 기본 탄환 속도 +${percent(codexSpeed)}`:`도감 ${n}개 발견`,shotSpeed:codexSpeed,moveSpeed:0,maxHp:0});
 ...
 const moveSpeedBonus=austin?AUSTIN_MOVE_SPEED:0,shotSpeedBonus=codexSpeed,maxHpBonus=alwaysBeginner?ALWAYS_BEGINNER_MAX_HP:0;
 return {titles,equipped:selected?.id||null,shown:selected?.name||null,moveSpeed:1+moveSpeedBonus,moveSpeedBonus,shotSpeed:1+shotSpeedBonus,shotSpeedBonus,shotSpeedSources:sources,maxHp:100+maxHpBonus,maxHpBonus,next};
}
