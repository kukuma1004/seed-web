...
export function titleState({austin=false,austinClear=false,austinVeteran=false,alwaysBeginner=false,alwaysClear=false,discovered=0,total=Infinity,badges=[],equipped=''}={}){
 ...
 const moveSpeedBonus=(austin?AUSTIN_MOVE_SPEED:0)+(austinVeteran?AUSTIN_VETERAN_MOVE_SPEED:0),shotSpeedBonus=0,maxHpBonus=alwaysBeginner?ALWAYS_BEGINNER_MAX_HP:0;
 const clearStatBonus=(Number(austinClear)+Number(alwaysClear))*CLEAR_ALL_STATS;
 return {titles,equipped:selected?.id||null,shown:selected?.name||null,moveSpeed:1+moveSpeedBonus,moveSpeedBonus,shotSpeed:1,shotSpeedBonus,shotSpeedSources:sources,attackCadence:1,attackCadenceBonus:0,maxHp:100+maxHpBonus+100*clearStatBonus,maxHpBonus,clearStatBonus,codexBonus:codex?codexStat:0,next};
}
