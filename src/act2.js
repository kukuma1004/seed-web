...
// Act 2 starts after an Austin clear, so its first journey must already sit
// above act 1's first journey. These multipliers raise pressure without adding
// extra render families or exceeding the existing 14-actor live cap.
export const ACT2_PRESSURE=Object.freeze({
 hp:1.15,speed:1.08,projectile:1.1,bossTempo:1.05,
 crowdInitial:10,crowdInterval:.76,crowdExtra:Object.freeze([2,3,4,6,0]),
 swarmSpeed:1.18,swarmDamage:12
});
...
export const STADIUM_ROOMS=Object.freeze([
 ...
 Object.freeze({name:'내야 다이아몬드',hint:'네 모서리를 순환하세요 · 마운드와 홈 사이의 흰 투구선을 베이스 활주로 가로지르세요',covers:[wall(-5.4,0,1,3.2),wall(5.4,0,1,3.2)],enemies:[['catcher',0,-3],['pitcher',0,-6],['runner',-5.7,-1.6],['runner',5.7,-1.6]]}),
 ...
 Object.freeze({name:'글러브 포켓',hint:'깊은 포켓은 안전해 보여도 막다른 길입니다 · 손가락 사이 홈을 짧게 쓰고 중앙으로 돌아오세요',covers:[wall(-5.55,2.1,1.5,1),wall(5.55,2.1,1.5,1)],enemies:[['batter',-4,-4],['batter',4,-4],['catcher',0,-2.4],['pitcher',-5.2,-5.7],['pitcher',5.2,-5.7]]}),
 Object.freeze({name:'결전의 야구장',hint:'넓은 외야를 쓰되 부채꼴 끝에 몰리지 마세요 · 문지기와 지원수를 함께 읽어야 합니다',covers:[wall(-6.6,2,2,1),wall(6.6,2,2,1)],enemies:[['act2warden',0,-3],['pitcher',-6.1,-5.2],['runner',6.1,-4.4]]})
]);
