...
export function createCameraFeel(){
 ...
 function follow(dt,{playerX=0,playerZ=0,moveX=0,moveZ=0,followX=.14,followZ=.06,leadScale=1,baseZoom=1,biasZ=0}={}){
  ...
  if(speed>.18){const length=Math.hypot(moveX,moveZ)||1,strength=clamp((speed-.18)/3.4,0,1)*clamp(leadScale,0,1);targetLeadX=moveX/length*.82*strength;targetLeadZ=moveZ/length*.54*strength;}
