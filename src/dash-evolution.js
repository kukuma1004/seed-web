...
export const DASH_EVOLUTIONS=Object.freeze({
 twinsprout:Object.freeze({id:'twinsprout',name:'쌍싹 회피',mark:'◇◇',tag:'두 번 연속',desc:'회피를 두 번 저장합니다. 빈 충전은 4.4초마다 하나씩 돌아옵니다.',strength:'빽빽한 탄막을 연속으로 빠져나감',tradeoff:'두 번을 모두 쓰면 완전히 차기까지 오래 걸림',charges:2,recharge:4.4,duration:.2,speed:17,invuln:.32}),
 longroot:Object.freeze({id:'longroot',name:'긴뿌리 도약',mark:'➶',tag:'거리 증가',desc:'한 번의 회피가 더 빠르고 38% 멀리 뻗어 나갑니다.',strength:'넓은 장판과 문지기 돌진에서 단숨에 벗어남',tradeoff:'방향을 잘못 잡으면 더 멀리 위험한 곳으로 들어감',charges:1,recharge:2.8,duration:.24,speed:19.55,invuln:.34}),
 molt:Object.freeze({id:'molt',name:'허물 벗기',mark:'◈',tag:'무적 증가',desc:'벗어 둔 허물 덕분에 회피가 끝난 뒤에도 무적이 더 오래 이어집니다(0.32초 → 0.5초).',strength:'정확한 순간에 쓰면 겹친 탄환과 폭발까지 넘김',tradeoff:'재사용 시간은 3초로 조금 길어짐',charges:1,recharge:3,duration:.2,speed:17,invuln:.5})
});
...
export function dashEvolutionCards(){
 return Object.values(DASH_EVOLUTIONS).map(d=>`<button class="dash-evolution-card" data-dash-evolution="${d.id}"><span class="dash-mark" aria-hidden="true">${d.mark}</span><small>${d.tag}</small><strong>${d.name}</strong><p>${d.desc}</p><b>${d.strength}</b><em>${d.tradeoff}</em></button>`).join('');
}

export function dashEvolutionSummary(state){
 const d=DASH_EVOLUTIONS[state?.id];if(!d)return '';
 return `<section class="item-bag dash-evolution-summary"><h3>회피 진화 · ${d.name}</h3><p><span class="dash-summary-mark">${d.mark}</span>${d.desc}<br><small>${d.strength} · ${d.tradeoff}</small></p></section>`;
}
