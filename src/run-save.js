...
// Three boss victories end on journey 15. Older checkpoints beyond that point
// retain their progress but use the last supported difficulty tier until the
// next boss ends the run.
...
export function difficulty(cycle){
 const c=Math.min(MAX_RUN_CYCLE,Math.max(0,Number(cycle)||0)),early=Math.min(c,20),late=Math.max(0,c-20);
