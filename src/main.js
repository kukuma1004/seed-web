...
import {ACTIVE_BUTTON_HTML,renderActiveButton} from './active-ui.js';
...
document.body.insertAdjacentHTML('beforeend','<button id="exit-room" hidden>다음 방으로 · E</button><div id="boss-hud" hidden><strong>기억의 문지기</strong><div><i></i></div><small></small></div><div id="score-hud" hidden><small>점수</small><b>0</b></div><div id="item-bar" hidden role="group" aria-label="물약 가방"></div>'+ACTIVE_BUTTON_HTML+'<div id="item-status" hidden aria-live="polite"></div>');
...
function offerSolo(onDone=finishChoice,force=false){
 const candidates=soloReady(levels).filter(f=>force||!promptedSolo.has(f.id+':'+levels.get(f.requires[0])));
...
function wave(){
 ...
 clearForms();cancelActive(activeGauge);finaleEchoes.length=0;cachedTarget=null;targetTimer=0;clearEscorts();escortWaves=0;bossDefeated=false;vfx.clear();wells.length=0;roomStartKills=kills;midReward=false;trapClock=0;pulls.length=0;orbitHits.clear();dashLock=0;crowdLeft=CROWD_TOTALS[stage];crowdTimer=0;crowdIndex=0;orbitHitCD=0;
...
function useActive(){
 ...
 if(!plan){const s=activeState(heldForms);$('#toast').textContent=s.state==='LOCKED'?'궁극기는 완성 진화나 단독 진화를 얻으면 열립니다':activeGauge.plan?'궁극기가 이미 발동 중입니다':`궁극기 게이지 ${Math.floor(activeGauge.value)}/${ACTIVE.max} · 적을 처치해 채우세요`;return;}
 ...
 cameraShake=Math.max(cameraShake,plan.state==='OVERDRIVE'?.35:.2);
...
function updateActive(dt){
 ...
 if(done){for(const id of done.forms)formCombats.get(id)?.calm();if(done.finale&&!roomCleared)overdriveBlast(done.finale,done.finale.radius,done.finale.damage,true);}
...
function overdriveBlast(f,radius,damage,first){
 ...
 vfx.pulse(center,f.slow?'frost':'amber',radius,.6);vfx.burst(center,'amber',first?48:24,radius*.5);cameraShake=Math.max(cameraShake,first?.45:.25);
...
function restart(saved=null){const restore=saved?.version===1?saved:null;if(!restore)clearCheckpoint(runStorage);heldForms.clear();rerollUsed=restore?.rerollUsed===true;if(restore)guideTarget=profile.forms.includes(restore.guideTarget)?restore.guideTarget:null;promptedForms.clear();clearEscorts();vfx.clear();wells.length=0;orbitGroup.visible=false;touch.reset();for(let e of enemies)releaseEnemy(e);for(const f of fallen)releaseEnemy(f.e);fallen.length=0;for(let p of [...shots,...enemyShots,...effects])release(p.ob);enemies=[];shots=[];enemyShots=[];effects=[];levels.clear();syncLaws();choicesTaken=0;choiceKills=0;dashLock=0;pulls.length=0;orbitHits.clear();chosen.clear();mutated.clear();roomCleared=false;exitOpen=false;growth.reset();playerMotion.reset();player.visible=true;evolutionTime=0;$('#evolution').hidden=true;updateFormLabel();document.querySelectorAll('#rules>div').forEach(n=>n.classList.remove('active'));hp=100;playerSlow=0;dash=0;invuln=1;shootCD=0;keyboardDash=false;keys.clear();player.userData.dashTime=0;stage=0;kills=0;elapsed=0;player.position.set(0,0,5);mode='playing';paused=false;$('#overlay').hidden=true;$('#pause').textContent='Ⅱ';$('#toast').textContent='';$('#overlay').classList.remove('intro');lastMove.set(0,0,1);cycle=restore?.cycle||0;region=restore?.region||'garden';score=restore?.score||0;wardensDefeated=restore?.wardens||0;austinsDefeated=restore?.austins||0;inventory=normalizeInventory(restore?.inventory);hasteTime=0;shellTime=0;selectedItem=null;itemBarKey='';relics=normalizeRelics(restore?.relics);relicRewardPending=false;austinRoom=false;potionCD=0;
 ...
 activeGauge=createActiveGauge(restore?.activeGauge||0);finaleEchoes.length=0;promptedSolo.clear();
...
if(localInspection&&new URLSearchParams(location.search).has('bossLab')){
 ...
 lab.innerHTML='<span>로컬 합성 검사 · 랭킹 전송 없음</span><button id="lab-hurt">보스 다음 단계</button><button id="lab-heal">검사 생명 회복</button><button id="lab-safe">검사 보호 꺼짐</button>';
 document.body.append(lab);lab.querySelector('#lab-hurt').onclick=()=>window.seedDebug.qa.hurtBoss(1);lab.querySelector('#lab-heal').onclick=()=>window.seedDebug.qa.setHp(100);lab.querySelector('#lab-safe').onclick=()=>{labSafe=!labSafe;lab.querySelector('#lab-safe').textContent=labSafe?'검사 보호 켜짐':'검사 보호 꺼짐';};
}
