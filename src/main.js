...
import {createSeedTitle,titleShotSpeed,AUSTIN_TITLE,AUSTIN_TITLE_PERK} from './seed-title.js';
...
const pauseBuild=createPauseBuild($('#save-exit'),()=>togglePause(),{get:()=>relics,effect:relicFx,canSwap:()=>roomCleared&&exitOpen,swap:id=>{if(!roomCleared||!exitOpen||!equipRelic(relics,id))return;syncLaws();if(stage===4)saveAfterBoss();else saveBoundary(stage+1);}},{get:()=>inventory},{get:()=>activeGauge},{unlocked:()=>seedTitle.isUnlocked()});
