...
let last=performance.now(),realLast=Date.now(),paceGame=0,paceReal=0,frames=[],frameCounter=0;function animate(now){requestAnimationFrame(animate);let raw=(now-last)/1000;last=now;
 ...
 shotBatches.sync(shots,enemyShots);perfMark(PS.scene);if(bloomPass.enabled)composer.render();else renderer.render(renderPass.scene,renderPass.camera);perfMark(PS.render);if(inspection&&frameCounter%30===0)inspection.textContent=JSON.stringify(window.seedDebug.getState());}
