import {FORMS,eligibleForms,isFormEligible,formLevel,formUpgradeLine} from './forms.js';
...
import {SLOT_CAP,killsForChoice,levelOf,damageScale,lawStats,offerChoices,chooseLaw,levelsFromSave,levelsToSave,totalLevel,upgradeLine} from './progression.js';
...
function levelPressure(){return 1+.07*Math.max(0,totalLevel(levels)-1);}
function syncForm(){if(activeForm&&!isFormEligible(activeForm,chosen))activeForm=null;formCombat.set(activeForm,formLevel(levels,activeForm));updateFormLabel();}
...
function offerForm(force=false,onDone=finishChoice){
 ...
 $('#overlay').innerHTML=`<p>두 법칙이 하나의 공격으로</p><h2>완성 진화를 선택하세요</h2><p>한 번에 한 진화만 사용합니다. 공격 방식과 강점·약점이 함께 바뀝니다.</p><div class="form-cards">${candidates.map(f=>formCard(f,[...chosen],profile.forms.includes(f.id),true,formLevel(levels,f.id))).join('')}</div><p class="form-note">완성 진화가 기본 탄환을 대신합니다. 재료 두 법칙을 강화할수록 진화 레벨이 오릅니다.<br>재료가 아닌 연쇄·폭발·중력·빙결·공전은 보조 효과로 남습니다. 보상 화면에서 기본 공격으로 돌아갈 수 있어요.</p><button id="keep-form" class="primary">${activeForm?FORMS[activeForm].name+' 유지':'기본 조합 유지'}</button>`;
...
const formCombat=createFormCombat(scene,{player,enemies:()=>enemies,hit:formHit,blocked:(a,b)=>segmentHitsCover(a,b,obstacles,.1),boundary:(a,b,d)=>reflectArenaBoundary(a,b,d,arena),constrain:collide,vfx,enemyShots:()=>enemyShots});
...
function saveBoundary(nextStage=stage,saveMode='entry'){
 saveOK=writeCheckpoint(runStorage,{version:1,cycle,region,stage:nextStage,mode:saveMode,hp,rules:[...chosen],mutated:[...mutated],levels:levelsToSave(levels),choicesTaken,choiceKills,kills,elapsed,form:activeForm,guideTarget,rerollUsed});
 return saveOK;
}
...
function wave(){
 formCombat.clear();formCombat.set(activeForm,formLevel(levels,activeForm));clearEscorts();escortWaves=0;bossDefeated=false;vfx.clear();wells.length=0;roomStartKills=kills;midReward=false;trapClock=0;pulls.length=0;orbitHits.clear();dashLock=0;crowdLeft=CROWD_TOTALS[stage];crowdTimer=0;crowdIndex=0;orbitHitCD=0;
 ...
 for(const spot of turretSpots(stage,cycle)){const t=createTurret(scene,mats,copiedLaws(levels));t.g.position.set(spot.x,0,spot.z);enemies.push(t);}
...
function cardChoice(mid=false,fixedOffer=null){
 ...
 let offered=fixedOffer||offerChoices(levels,{guide});
 if(!offered.length){mode='playing';if(roomCleared)openExit();return;}
 $('#overlay').hidden=false;$('#overlay').innerHTML=`<p>${mid?'처치 게이지 가득':'문지기 격파 보상'} · 슬롯 ${chosen.size}/${SLOT_CAP}</p><h2>${chosen.size>=SLOT_CAP?'법칙을 더 깊게':chosen.size?'어떤 씨앗으로 자랄까요':'첫 법칙이 깨어납니다'}</h2><p>${chosen.size>=SLOT_CAP?'슬롯이 가득 찼습니다 · 가진 법칙을 끝없이 강화합니다':'슬롯 '+SLOT_CAP+'개를 채운 뒤에는 강화만 합니다'} · 다음 선택까지 ${killsForChoice(choicesTaken)} 처치</p><div class="build-preview">${[...chosen].map(id=>`<span class="build-law">${lawArt(id)}${LAWS[id].name} Lv.${levelOf(levels,id)}</span>`).join('<span class="build-link">◇</span>')||'아직 이름 없는 시드'}</div><div class="cards">${offered.map(id=>{const held=chosen.has(id),v=LAWS[id];return `<button class="card" data-choice="${id}" style="--law-color:#${v.color.toString(16).padStart(6,'0')}">${lawArt(id,'card-art')}<strong>${v.name} Lv.${held?levelOf(levels,id)+1:1}</strong><p>${held?'강화 Lv.'+levelOf(levels,id)+' → '+(levelOf(levels,id)+1):'새로운 법칙'}</p><small>${held&&activeForm&&FORMS[activeForm].requires.includes(id)?formUpgradeLine(activeForm,formLevel(levels,activeForm)):held?upgradeLine(levels,id)+' · 모든 탄 피해 +10%':v.desc}</small><span class="synergy">${formLawHint(id,activeForm,formLevel(levels,activeForm))||synergyHint(id,[...chosen])}</span></button>`;}).join('')}</div>`;
 const goals=Object.values(FORMS).filter(f=>f.id!==activeForm&&f.requires.filter(id=>chosen.has(id)).length===1).slice(0,4).map(f=>`${f.name}까지 ${LAWS[f.requires.find(id=>!chosen.has(id))].name}`).join(' · ');
 $('#overlay').insertAdjacentHTML('beforeend',`<div class="form-progress">${activeForm?'현재 진화: '+FORMS[activeForm].name+' Lv.'+formLevel(levels,activeForm)+' · ':''}${goals||'법칙 두 개를 맞추면 완성 진화 · 발견 '+profile.forms.length+'/'+Object.keys(FORMS).length}</div><div class="form-actions">${rerollUnlocked(profile)&&!rerollUsed?'<button class="primary" id="reroll-laws">선택지 새로고침 · 이번 여정 1회</button>':''}${eligibleForms(chosen).length?'<button class="primary" id="change-form">현재 조합의 진화 보기</button>':''}${activeForm?'<button class="primary" id="base-form">기본 탄환 조합으로</button>':''}</div>`);
 ...
 document.querySelectorAll('[data-choice]').forEach(button=>button.onclick=()=>{if(mode!=='cards')return;const id=button.dataset.choice;if(!chooseLaw(levels,id))return;syncLaws();beginEvolution(id);});
}

function updateFormLabel(){ $('#form-label').textContent=activeForm?FORMS[activeForm].name+' Lv.'+formLevel(levels,activeForm)+' · '+FORMS[activeForm].strength:[...chosen].map(id=>LAW_PRESENTATION[id].name+' Lv.'+levelOf(levels,id)).join(' · ')||'아직 이름 없는 씨드'; }
function beginEvolution(id){
  syncForm();formCombat.clear();formCombat.set(activeForm,formLevel(levels,activeForm));
...
function restart(saved=null){const restore=saved?.version===1?saved:null;if(!restore)clearCheckpoint(runStorage);activeForm=null;rerollUsed=restore?.rerollUsed===true;if(restore)guideTarget=profile.forms.includes(restore.guideTarget)?restore.guideTarget:null;promptedForms.clear();formCombat.clear();clearEscorts();vfx.clear();wells.length=0;orbitGroup.visible=false;touch.reset();for(let e of enemies)release(e.g);for(let p of [...shots,...enemyShots,...effects])release(p.ob);enemies=[];shots=[];enemyShots=[];effects=[];levels.clear();syncLaws();choicesTaken=0;choiceKills=0;dashLock=0;pulls.length=0;orbitHits.clear();chosen.clear();mutated.clear();roomCleared=false;exitOpen=false;growth.reset();playerMotion.reset();player.visible=true;evolutionTime=0;$('#evolution').hidden=true;updateFormLabel();document.querySelectorAll('#rules>div').forEach(n=>n.classList.remove('active'));hp=100;playerSlow=0;dash=0;invuln=1;shootCD=0;keyboardDash=false;keys.clear();player.userData.dashTime=0;stage=0;kills=0;elapsed=0;player.position.set(0,0,5);mode='playing';paused=false;$('#overlay').hidden=true;$('#pause').textContent='Ⅱ';$('#toast').textContent='';$('#overlay').classList.remove('intro');lastMove.set(0,0,1);cycle=restore?.cycle||0;region=restore?.region||'garden';
 if(restore){stage=restore.stage;hp=restore.hp;kills=restore.kills;elapsed=restore.elapsed;for(const [id,v] of levelsFromSave(restore))levels.set(id,v);syncLaws();choicesTaken=restore.choicesTaken||0;choiceKills=restore.choiceKills||0;growth.select(chosen,mutated);growth.update(0,0,false);updateFormLabel();}
...
if(autoTarget&&shootCD<=0&&!roomCleared&&!(activeForm&&FORMS[activeForm].passive)){if(activeForm){shootCD=formCombat.fire(player.position,aimdir,autoTarget.g.position);growth.fire();}else{shootCD=.22;growth.fire();vfx.muzzle(player.position.clone().addScaledVector(aimdir,.65),aimdir,[...chosen][0]||'seed');projectile(player.position.clone().addScaledVector(aimdir,.6),aimdir);}}
...
orbitGroup.visible=chosen.has('orbit')&&!(activeForm&&FORMS[activeForm].requires.includes('orbit'));orbitTime+=dt*3.1;
if(orbitGroup.visible){for(const [e,t] of orbitHits){if(e.dead||t<=dt)orbitHits.delete(e);else orbitHits.set(e,t-dt);}
 orbitGroup.children.forEach((o,i)=>{o.visible=i<LS.orbitPetals;if(!o.visible)return;const a=orbitTime+i*Math.PI*2/LS.orbitPetals;o.position.set(player.position.x+Math.cos(a)*LS.orbitRadius,.7,player.position.z+Math.sin(a)*LS.orbitRadius);o.rotation.y=-a;
  for(const e of enemies)if(!e.dead&&!orbitHits.has(e)&&Math.hypot(e.g.position.x-o.position.x,e.g.position.z-o.position.z)<(e.type==='warden'?1.5:.95)){orbitHits.set(e,.28);const push=e.g.position.clone().sub(player.position).setY(0).normalize();if(activeForm)formHit(e,LS.orbitDamage,{kind:activeForm,indirect:true});else applyLawHit(e,LS.orbitDamage);if(!e.dead&&e.type!=='warden'&&e.type!=='turret'){e.g.position.addScaledVector(push,.4);collide(e.g.position,.4);}}
...
updateEscorts(0);
enemies=enemies.filter(e=>!e.dead);if(!enemies.length&&crowdLeft===0&&mode==='playing'&&!roomCleared){for(let p of enemyShots)release(p.ob);enemyShots=[];roomCleared=true;formCombat.clear();for(const p of shots)release(p.ob);shots=[];if(stage<4)openExit();else cardChoice();}
...
function updateGauges(){
 ...
 $('#growth-progress span').textContent=`다음 법칙까지 ${need-have} 처치 · 슬롯 ${chosen.size}/${SLOT_CAP}`;$('#growth-progress i').style.width=(have/need*100)+'%';
