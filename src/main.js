...
import {MIRROR_ATTACK_CADENCE,MIRROR_BREAK,MIRROR_TRIAL_PROTOTYPE,mirrorAttackCooldown,mirrorFloorRules,refundMirrorAttackCooldown} from './mirror-trial.js';
...
function spawnAct3(type,x,z){const e=createAct3Minion(scene,type,rng),art=ACT3_ART[type];e.g.position.set(x,0,z);attachActorArt(e,camera,release,{file:art.file,size:art.size,atlasFrame:art.frame,topDownFacing:true,baseline:art.baseline,occlusion:false,lighting:false});enemies.push(e);return e;}
...
function wave(){
 ...
 if(mirrorSession){
  const e=createMirrorFighter(scene,{floor:mirrorSession.floor,quality:qualityLevel===0?'low':'normal',levels,forms:heldForms,dashEvolution:dashState.id});e.g.position.set(0,0,-6.8);enemies.push(e);mirrorSession.enemy=e;mirrorSession.cracks=0;$('#boss-hud strong').textContent=`거울의 탑 ${mirrorSession.floor}층 · 비친 씨앗`;
   ...
   if(isAct3(region)){const e=createTempestCarrier(scene),art=ACT3_ART.tempestcarrier;e.g.position.set(0,0,-1.8);attachActorArt(e,camera,release,{file:art.file,size:art.size,atlasFrame:actor=>art.frames[actor.phaseIndex||0],topDownFacing:true,baseline:art.baseline,occlusion:false,lighting:false});enemies.push(e);$('#boss-hud strong').textContent=TEMPEST_CARRIER.name;}
...
function showMirrorTrialResult(won,report=null){
 const floor=mirrorSession?.floor||1,perfect=mirrorSession?.perfectDodges||0;mode='mirror-result';paused=false;touch.reset();keys.clear();mirrorReadyRing.visible=false;for(const p of enemyShots)release(p.ob);enemyShots=[];$('#boss-hud').hidden=true;$('#overlay').hidden=false;$('#overlay').classList.add('intro','menu-screen','developer-mode');
 $('#overlay').innerHTML=`<div class="menu-panel developer-panel"><p class="eyebrow">SEED · MIRROR TOWER PROTOTYPE</p><h2>${won?'거울의 탑 시제품 완주':'거울에 잠든 씨앗'}</h2><p>${won?'10층까지 같은 선택을 나눈 분신을 이겼습니다.':`${floor}층에서 시련이 끝났습니다.`}</p><div class="final-score"><small>로컬 검사 기록</small><strong>${floor}층</strong><span>완벽 회피 ${perfect}회 · 점수와 보상은 저장되지 않음</span></div>${report?combatAnalysisSummary(report):''}<div class="developer-footer"><button class="primary" id="mirror-retry">1층부터 다시</button><button id="mirror-end">메인으로</button></div></div>`;
 $('#mirror-retry').onclick=startMirrorTower;$('#mirror-end').onclick=showIntro;
}
...
function startMirrorTower(){
 developerRun=true;labSafe=false;startRegion='garden';mirrorSession={floor:1,cracks:0,perfectDodges:0,enemy:null};restart();
}
...
if(localInspection&&new URLSearchParams(location.search).has('act3Lab'))setTimeout(()=>{const value=new URLSearchParams(location.search).get('act3Lab');if(value==='boss')window.seedDebug?.qa.startAct3Boss();else window.seedDebug?.qa.startAct3Stage(Number(value)||0);},1200);
