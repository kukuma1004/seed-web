...
// FORMS here means every evolution the seed can hold: 45 first fusions, ten solo evolutions and their awakenings.
...
import {RUN_BONUSES,emptyRunBonuses,normalizeRunBonuses,runBonusOffers,applyRunBonus,runBonusSummary,moveScale as runMoveScale,shotScale as runShotScale,powerScale as runPowerScale} from './run-bonuses.js';
...
let score=0,wardensDefeated=0,austinsDefeated=0,austinRoom=false,inventory=emptyInventory(),runBonuses=emptyRunBonuses(),turretPotionDry=0,relics=emptyRelics(),relicRewardPending=false,dashRewardPending=false,clockFloor=null,potionCD=0;const fallen=[];const JOURNEY_HEAL=25;
...
if(localInspection&&new URLSearchParams(location.search).has('gardenLab'))garden=normalizeGarden({...garden,harvests:30,plots:[
 {seed:'reflect',growth:0,branch:null,active:false},{seed:'split',growth:2,branch:null,active:false},
 {seed:'chain',growth:4,branch:'flower',active:false},{seed:'orbit',growth:9,branch:'tree',active:false},
 {seed:'gravity',growth:9,branch:'vine',active:false},{seed:'frost',growth:9,branch:'flower',active:false}
]});
...
function formHit(e,amount,meta){
 ...
 const scale=damageScale(levels)*relicFormScale(relics,consumed)*runPowerScale(runBonuses);damageEnemy(e,amount*scale,false);twinResonance(e,amount,meta,scale);
...
function formOptions(){return {player,enemies:()=>enemies,hit:formHit,blocked:(a,b)=>segmentHitsCover(a,b,obstacles,.1),boundary:(a,b,d)=>reflectArenaBoundary(a,b,d,arena),constrain:collide,vfx,sound:id=>audio.play(id),enemyShots:()=>enemyShots,theme:combatTheme};}
 ...
 for(let i=runes.length-1;i>=0;i--){
  ...
  if(r.tick<=0){
   ...
   for(const e of enemies)if(!e.dead&&Math.hypot(e.g.position.x-r.pos.x,e.g.position.z-r.pos.z)<RUNE.radius)
    damageEnemy(e,RUNE.damage*damageScale(levels)*runPowerScale(runBonuses),false);
  }
 }
}
...
function overdriveBlast(f,radius,damage,first){
 const center=player.position.clone().setY(0),scale=damageScale(levels)*runPowerScale(runBonuses),flatDistance=e=>Math.hypot(e.g.position.x-center.x,e.g.position.z-center.z);
...
function hitPlayer(amount){if(labSafe||invuln>0||shellTime>0||mode!=='playing')return;hp=Math.max(0,hp-amount);invuln=.65;burst(player.position,'amber',16);cameraShake=.18;audio.play('hurt');
 if(hp<=0){
...
function runBonusCard(id){const bonus=RUN_BONUSES[id],level=id==='heal'?0:runBonuses[id]||0,next=id==='heal'?`현재 생명력 ${Math.ceil(hp)} / 100`:`${bonus.desc} · ${level+1}/5단계`;return `<button class="run-bonus-card" data-run-bonus="${id}" style="--bonus-color:${bonus.color}"><span aria-hidden="true">${bonus.icon}</span><div><small>법칙 대신 작은 성장</small><strong>${bonus.name}</strong><p>${next}</p></div></button>`;}
function cardChoice(mid=false,fixedOffer=null){
 ...
 const smallGrowth=runBonusOffers(runBonuses,{hp,choicesTaken});
 ...
 document.querySelectorAll('[data-run-bonus]').forEach(button=>button.onclick=()=>{if(mode!=='cards')return;const id=button.dataset.runBonus,result=applyRunBonus(runBonuses,id,{hp});if(!result.ok)return;runBonuses=result.state;hp=result.hp;const bonus=RUN_BONUSES[id];vfx.pulse(player.position,id==='heal'?'seed':id==='move'?'orbit':id==='shot'?'frost':'burst',1.7,.45);vfx.burst(player.position,id==='heal'?'seed':id==='move'?'orbit':id==='shot'?'frost':'burst',20,1.35);audio.play('pickup');finishChoice();$('#toast').textContent=id==='heal'?`${bonus.name} · 생명력 +${result.healed}`:`${bonus.name} ${result.level}/5 · ${bonus.desc}`;});
...
function showEnd(){touch.reset();$('#overlay').classList.remove('intro','menu-screen','garden-mode');activeVfx.clear();cancelActive(activeGauge);$('#active-cinematic').hidden=true;$('#active-cinematic').innerHTML='';$('#item-bar').hidden=true;$('#active-skill').hidden=true;$('#item-status').hidden=true;
 ...
 lastHarvest=harvestFromRun({levels:Object.fromEntries(effectiveLevels(levels,heldForms)),forms:Object.fromEntries(heldForms),wardens:wardensDefeated,austins:austinsDefeated,score,kills,journey:cycle+1});
...
if(mode!=='playing'||paused){player.userData.updateArt(camera);return;}const previousPlayer=player.position.clone();potionCD=Math.max(0,potionCD-dt);hasteTime=Math.max(0,hasteTime-dt);shellTime=Math.max(0,shellTime-dt);playerSlow=Math.max(0,playerSlow-dt);tickDash(dashState,dt);invuln=Math.max(0,invuln-dt);shootCD-=dt;const move=new V();if(keys.has('KeyW')||keys.has('ArrowUp'))move.z-=1;if(keys.has('KeyS')||keys.has('ArrowDown'))move.z+=1;if(keys.has('KeyA')||keys.has('ArrowLeft'))move.x-=1;if(keys.has('KeyD')||keys.has('ArrowRight'))move.x+=1;move.add(new V(touch.axes.move.x,0,touch.axes.move.y)).clampLength(0,1);if(move.lengthSq()>.001)lastMove.copy(move).normalize();dashLock=Math.max(0,dashLock-dt);const touchDash=touch.consumeDash(),dashRequested=keyboardDash||touchDash;keyboardDash=false;const dashMove=dashRequested&&dashLock<=0?spendDash(dashState):null;if(dashMove){invuln=Math.max(invuln,dashMove.invuln);player.userData.dashTime=dashMove.duration;player.userData.dashSpeed=dashMove.speed;player.userData.dashDir=lastMove.clone();vfx.burst(player.position,dashState.id==='molt'?'reflect':'seed',dashState.id==='molt'?22:14);vfx.dash(player.position,player.rotation.y);audio.play('dash');player.userData.dashFX=0;}if(player.userData.dashTime>0){player.userData.dashTime-=dt;player.position.addScaledVector(player.userData.dashDir,dt*(player.userData.dashSpeed||17));player.userData.dashFX+=dt;if(player.userData.dashFX>=.045){player.userData.dashFX=0;vfx.dash(player.position,player.rotation.y);}}else player.position.addScaledVector(move,dt*PLAYER_SPEED*runMoveScale(runBonuses)*(playerSlow>0?.75:1)*(hasteTime>0?ITEMS.wind.speed:1));collide(player.position);aimRing.visible=false;targetTimer-=dt;if(targetTimer<=0||!cachedTarget||cachedTarget.dead){cachedTarget=acquireTarget(player.position,enemies,obstacles,segmentHitsCover);targetTimer=.1;}const autoTarget=cachedTarget;if(autoTarget)aim.copy(autoTarget.g.position);let aimdir=aim.clone().sub(player.position).setY(0).normalize();player.rotation.y=Math.atan2(aimdir.x,aimdir.z);playerMotion.update(dt,player.position.x-previousPlayer.x,player.position.z-previousPlayer.z,{dashing:player.userData.dashTime>0});player.userData.updateArt(camera,player.position.x-previousPlayer.x,player.position.z-previousPlayer.z);player.visible=invuln<.5||Math.floor(time*20)%2===0;player.userData.halo.rotation.z=time*.4;
if(autoTarget&&shootCD<=0&&!roomCleared){shootCD=.22;growth.fire();vfx.muzzle(player.position.clone().addScaledVector(aimdir,.65),aimdir,[...chosen][0]||'seed');projectile(player.position.clone().addScaledVector(aimdir,.6),aimdir);audio.play('shot');}
for(const [id,combat] of formCombats){const cd=(formCooldowns.get(id)||0)-dt;if(autoTarget&&cd<=0&&!roomCleared&&!FORMS[formAttacks.get(id)||id]?.passive){formCooldowns.set(id,combat.fire(player.position,aimdir,autoTarget.g.position));audio.play('shot');}else formCooldowns.set(id,Math.max(-1,cd));}
