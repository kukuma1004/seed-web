...
document.body.insertAdjacentHTML('beforeend','<div id="growth-progress"><span></span><div><i></i></div></div><div id="overhead" hidden><div class="oh-hp"><i></i></div><div class="oh-ammo"></div><div class="oh-dash"><i></i></div><b class="oh-lock" hidden>회피 봉인</b></div>');
...
function wave(){
 formCombat.clear();formCombat.set(activeForm);clearEscorts();escortWaves=0;bossDefeated=false;vfx.clear();wells.length=0;roomStartKills=kills;midReward=false;trapClock=0;pulls.length=0;orbitHits.clear();dashLock=0;crowdLeft=CROWD_TOTALS[stage];crowdTimer=0;crowdIndex=0;orbitHitCD=0;
...
function updateFormLabel(){ $('#form-label').textContent=activeForm?FORMS[activeForm].name+' · '+FORMS[activeForm].strength:[...chosen].map(id=>LAW_PRESENTATION[id].name+' Lv.'+levelOf(levels,id)).join(' · ')||'아직 이름 없는 씨드'; }
...
if(autoTarget&&shootCD<=0&&!roomCleared&&activeForm!=='frostguard'){if(activeForm){shootCD=formCombat.fire(player.position,aimdir);growth.fire();}else if(reload<=0){shootCD=.2;ammo--;growth.fire();vfx.muzzle(player.position.clone().addScaledVector(aimdir,.65),aimdir,[...chosen][0]||'seed');projectile(player.position.clone().addScaledVector(aimdir,.6),aimdir);if(ammo<=0)reload=AMMO_RELOAD;}}
if(reload>0){reload-=dt;if(reload<=0){reload=0;ammo=AMMO_MAX;}}else if(!autoTarget&&ammo<AMMO_MAX){ammoRegen-=dt;if(ammoRegen<=0){ammo++;ammoRegen=.3;}}
...
orbitGroup.visible=chosen.has('orbit')&&activeForm!=='frostguard';orbitTime+=dt*3.1;
...
function updateGauges(){
 ...
 const clip=$('#overhead .oh-ammo');clip.classList.toggle('reloading',reload>0);clip.classList.toggle('hidden-ammo',Boolean(activeForm));
 clip.innerHTML=reload>0?`<i style="width:${(1-reload/AMMO_RELOAD)*100}%"></i>`:Array.from({length:AMMO_MAX},(_,k)=>`<b class="${k<ammo?'on':''}"></b>`).join('');
