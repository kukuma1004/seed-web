...
function showDeveloperLab(){
 ...
 $('#developer-usage').onclick=showDeveloperUsage;
...
function showDeveloperUsage(){
 ...
 $('#overlay').classList.remove('ranking-overlay','garden-mode');$('#overlay').classList.add('intro','menu-screen','developer-mode');$('#overlay').hidden=false;
 $('#overlay').innerHTML='<div class="menu-panel developer-panel usage-panel"><p class="eyebrow">SEED · ADMIN ONLY</p><h2>이용 현황</h2><div id="usage-content" role="status">최근 7일 수치를 불러오는 중…</div><div class="developer-footer"><button id="usage-refresh" type="button">새로고침</button><button id="usage-back" type="button">실험실로</button></div></div>';
 const minutes=seconds=>`${Math.round(seconds/60).toLocaleString('ko-KR')}분`;
 const refresh=async()=>{
  const target=$('#usage-content');if(!target)return;target.textContent='최근 7일 수치를 불러오는 중…';
  try{
 ...
 $('#usage-refresh').onclick=refresh;$('#usage-back').onclick=showDeveloperLab;void refresh();
}
...
function animate(now){animationHandle=0;if(document.hidden)return;startAnimation();let raw=(now-last)/1000;last=now;
 {const realNow=Date.now();const realDelta=(realNow-realLast)/1000;realLast=realNow;if(mode==='playing'&&!paused&&realDelta>0&&realDelta<2){elapsed+=realDelta;paceReal+=realDelta;paceGame+=Math.min(2,Math.max(0,raw));webTelemetry.playTick(realDelta);}}if(perfEnabled)try{perf.frame(raw*1000,mode==='playing'&&!paused,perfSnapshot);perf.beginFrame();if(perf.due())perf.sampleState(perfSnapshot());}catch{}frames.push(raw*1000);if(frames.length>180)frames.shift();const visualDt=Math.min(.1,Math.max(0,raw)),timeScale=cameraFeel.stepEffects(visualDt),dt=advanceFrame(raw*timeScale,now*.001,(step,time)=>update(step,time));audio.tick(visualDt);perfT=performance.now();if(!paused)vfx.update(visualDt);perfMark(PS.particles);frameCounter++;
