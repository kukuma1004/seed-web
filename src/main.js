...
import {ACT3_REGION,ACT3_NAME,ACT3_PRESSURE,isAct3,act3Unlocked,act3Available,playableAct3Region,act3Storage,act3CrowdType,act3ReinforcementSpawn,act3SupplyDrop} from './act3.js';
...
import {createProjectileSprites} from './projectile-sprites.js';
...
import {readCheckpointBackups,setCheckpointBackup} from './cloud-save.js';
...
const projectileSprites=createProjectileSprites(scene,camera,{mobile:mobileDevice,baseUrl:import.meta.env.BASE_URL,capacity:MAX_SHOTS+240,haloAtlas:vfxAtlas,onReady:texture=>renderer.initTexture(texture)});
if(localInspection&&new URLSearchParams(location.search).get('projectile3d')==='1')projectileSprites.setEnabled(false);
// ---------------- 셰이더 미리 준비 ----------------
...
function offerAwaken(onDone=finishChoice,force=false){
 ...
 $('#overlay').innerHTML=`<p>두 진화가 서로를 알아봅니다</p><h2>각성 진화</h2><p>융합 + 지정된 단독 진화는 융합 각성, 두 단독 진화는 쌍둥이 각성이 됩니다. 재료는 하나의 칸으로 합쳐집니다.</p><div class="form-cards">${options.map((o,i)=>awakenCard(o,heldForms,awakenLevel(heldForms,o),profile.forms.includes(o.id),i)).join('')}</div><p class="form-note">융합 각성은 융합 공격과 여는 기술을 강화합니다. 쌍둥이는 두 단독 공격이 함께 나갑니다.<br>지금 합치지 않아도 이후 카드 화면에서 다시 고를 수 있어요.</p><button id="keep-awaken" class="primary">지금은 각성하지 않기</button>`;
...
function wave(continueSkyway=false){
 ...
 clearForms();cancelActive(activeGauge);activeVfx.clear();finaleEchoes.length=0;cachedTarget=null;targetTimer=0;clearEscorts();escortWaves=0;bossDefeated=false;act2Support=null;bossRewardLine='';vfx.clear();wells.length=0;roomStartKills=kills;midReward=false;skywayAdvance=0;trapClock=0;pulls.length=0;orbitHits.clear();dashLock=0;baseSlideTime=baseSlideCooldown=0;baseSlideLock=-1;crowdLeft=mirrorSession||inAustinRoom()?0:crowdTotal(stage,cycle)+(isAct3(region)?ACT3_PRESSURE.crowdExtra[stage]||0:isAct2(region)?ACT2_PRESSURE.crowdExtra[stage]||0:0);crowdTimer=0;crowdIndex=0;orbitHitCD=0;
...
function useExit(forceSkyway=false){if(!(forceSkyway===true&&isAct3(region)&&exitOpen)&&!canUseExit({open:exitOpen,mode,paused,x:player.position.x,z:player.position.z,exit:arena.exit||EXIT}))return;touch.reset();keys.clear();keyboardDash=false;exitOpen=false;gate.visible=false;$('#exit-room').hidden=true;$('#toast').textContent='';if(stage===4){if(finalBossAhead())enterAustin();else nextJourney();}else{stage++;wave(isAct3(region));}}
...
function cardChoice(mid=false,fixedOffer=null){
 ...
 if(!offered.length){mode='playing';if(roomCleared){if(mirrorSession)advanceMirrorFloor();else openExit();}return;}
...
function showAccount(error=''){
 ...
 const busy=async(action,provider)=>{document.querySelectorAll('.account-button').forEach(button=>button.disabled=true);try{await action();}catch(err){void webTelemetry.loginFailure(provider,err);showAccount(authMessage(err));return;}try{const result=await cloud.retry();await refreshAccessMode();if(result?.changed){location.reload();return;}showEntry();}catch(err){showAccount(authMessage(err));}};
...
function showDungeon(){
 ...
 $('#overlay').innerHTML=`<div class="menu-panel dungeon-panel">
  ...
  <div class="menu-list">
   ...
   ${restoreRuns.map(r=>`<button id="restore-${r.act}" class="menu-item small-item restore-run"><strong>다른 기기 저장에 밀린 ${r.label} 판 되살리기</strong><small>${escapeHtml(r.detail)} · 지금 판과 맞바꿔요</small></button>`).join('')}
...
let last=performance.now(),realLast=Date.now(),paceGame=0,paceReal=0,frames=[],frameCounter=0;function animate(now){requestAnimationFrame(animate);let raw=(now-last)/1000;last=now;
 ...
 {const laws=chosen.values(),first=laws.next().value||'seed';auraList.length=0;for(const p of shots)auraList.push(p);for(const c of formCombats.values())c.auraBolts?.(auraList);projectileSprites.sync(shots,enemyShots);shotBatches.sync(shots,enemyShots);shotAuras.sync(auraList,camera,{theme:combatTheme,first,second:laws.next().value,bodyArt:'spriteHidden'});}perfMark(PS.scene);composer.render();perfMark(PS.render);if(inspection&&frameCounter%30===0)inspection.textContent=JSON.stringify(window.seedDebug.getState());}
...
if(localInspection&&new URLSearchParams(location.search).has('bossLab')){
 ...
 const refreshShotArt=()=>{auraList.length=0;for(const p of shots)auraList.push(p);for(const combat of formCombats.values())combat.auraBolts?.(auraList);projectileSprites.sync(shots,enemyShots);shotBatches.sync(shots,enemyShots);const laws=chosen.values();shotAuras.sync(auraList,camera,{theme:combatTheme,first:laws.next().value||'seed',second:laws.next().value,bodyArt:'spriteHidden'});};
 ...
 const shotArtToggle=document.createElement('button');shotArtToggle.textContent='탄 원화 ON';
 shotArtToggle.onclick=()=>{const next=!projectileSprites.active();projectileSprites.setEnabled(next);refreshShotArt();shotArtToggle.textContent=next?'탄 원화 ON':'기존 3D ON';};lab.append(shotArtToggle);
