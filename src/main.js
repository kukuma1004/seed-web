...
const sharedGeometries=new Set([baseOrbitGeo,...Object.values(austinGeo),...Object.values(projectileGeos),...Object.values(ACTOR_ART_GEOMETRIES)]);
...
function resize(){const w=document.documentElement.clientWidth,viewHeight=document.documentElement.clientHeight,h=touch.enabled&&viewHeight>w?Math.max(250,viewHeight-160):viewHeight;renderer.setSize(w,h,false);renderer.domElement.style.width='100%';renderer.domElement.style.height=h+'px';camera.aspect=w/h;phoneView=touch.enabled&&w>h&&h<=520;const tabletView=touch.enabled&&w>h&&h>520;document.body.classList.toggle('phone-landscape',phoneView);camera.zoom=Math.min(1.18,camera.aspect/(touch.enabled&&h>w?1.25:.95))*(phoneView?PHONE_ZOOM:tabletView?TABLET_ZOOM:1);camera.updateProjectionMatrix();composer.setSize(w,h);sizeBloom(w,h);canvasRect=renderer.domElement.getBoundingClientRect();if(touch.enabled&&mode==='playing'&&!paused)togglePause();}window.addEventListener('resize',resize);applyQuality(qualityLevel,{save:false});mountQualityButton();showIntro();requestAnimationFrame(animate);
