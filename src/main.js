...
document.body.insertAdjacentHTML('beforeend','<button id="exit-room" hidden>다음 방으로 · E</button><div id="boss-hud" hidden><strong>기억의 문지기</strong><div><i></i></div><small></small></div><div id="score-hud" hidden><small>점수</small><b>0</b></div><div id="item-bar" hidden role="group" aria-label="물약 가방"></div><div id="item-status" hidden aria-live="polite"></div>');
...
const pauseBuild=createPauseBuild($('#save-exit'),()=>togglePause(),{get:()=>relics,effect:relicFx,canSwap:()=>roomCleared&&exitOpen,swap:id=>{if(!roomCleared||!exitOpen||!equipRelic(relics,id))return;syncLaws();if(stage===4)saveAfterBoss();else saveBoundary(stage+1);}},{get:()=>inventory});
...
function damageEnemy(e,amount,chaining=true){if(e.state==='recover')amount*=1.35;if(e.type==='austin'){if(damageAustin(e,amount)<=0)return;}else e.hp-=amount;e.hit=.14;vfx.impact(e.g.position,chaining?([...chosen][0]||'seed'):'chain');if(chaining&&chosen.has('chain')){let nearby=enemies.filter(o=>o!==e&&o.hp>0&&o.g.position.distanceTo(e.g.position)<4).sort((a,b)=>a.g.position.distanceTo(e.g.position)-b.g.position.distanceTo(e.g.position)).slice(0,LS.chainTargets);for(let o of nearby){line(e.g.position,o.g.position);damageEnemy(o,amount*(isBoss(o)?.325:.5),false);}}if(e.hp<=0&&!e.dead){e.dead=true;kills++;choiceKills++;vfx.impact(e.g.position,'amber',true);enemyDown(e);}}
...
function showEnd(){touch.reset();$('#item-bar').hidden=true;$('#item-status').hidden=true;
