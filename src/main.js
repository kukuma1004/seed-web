...
function drawRoom(){
 ...
 if(!isAct2(region)||mirrorRoom)stadium.setActive(false);if(!isAct3(region)||mirrorRoom)skyway.setActive(false);if(!mirrorRoom&&isAct2(region))stadium.setActive(true,arena,{stage,bossRoom});if(!mirrorRoom&&isAct3(region))skyway.setActive(true);
...
let seedTitle=null;
const maxPlayerHp=()=>mirrorSession?100:(gardenStats()?.maxHp||100)+(seedTitle?.state().maxHpBonus||0);
const gardenPower=()=>mirrorSession?1:gardenStats()?.power||1;
const gardenCooldown=()=>mirrorSession?1:gardenStats()?.cooldownRate||1;
const totalCritChance=()=>Math.min(.25,(LS?.critChance||0)+(mirrorSession?0:(gardenStats()?.critical||0)));
...
function mirrorBolt(pos,dir,spec={}){
 ...
 enemyShots.push({mirror:true,boss:true,ob,dir:dir.clone(),life:5,bounces:spec.bounces||0,speed:7.2*(spec.speedScale||1),damage,age:0,origin:pos.clone(),pierce:true,recall:Boolean(spec.recall),frost:Boolean(spec.frost),curve:spec.curve||0,law:spec.law||'pierce'});
}
...
function stadiumBolt(pos,dir,spec){const ob=new THREE.Mesh(stadiumBallGeo,mats.stadiumBall);ob.position.set(pos.x+dir.x*1.05,.65,pos.z+dir.z*1.05);ob.castShadow=false;scene.add(ob);enemyShots.push({boss:Boolean(spec.boss),ob,dir:dir.clone(),life:spec.life||4.8,bounces:Math.max(0,Math.floor(spec.bounces||0)),speed:spec.speed,damage:spec.damage,curve:spec.curve||0,age:0,origin:pos.clone(),pierce:Boolean(spec.pierce)});}
...
function startDeveloperEncounter(target){
 ...
 developerRun=true;startRegion=target.startsWith('act3')?ACT3_REGION:target.startsWith('act2')?ACT2_REGION:'garden';restart();
 ...
 else if(target==='act3field'){stage=1;cycle=0;wardensDefeated=0;austinsDefeated=0;austinRoom=false;wave();}
 else if(target==='act3warden'){stage=4;cycle=2;wardensDefeated=4;austinsDefeated=0;austinRoom=false;wave();}
 else if(target==='act3boss'){stage=4;cycle=3;wardensDefeated=5;austinsDefeated=0;austinRoom=true;wave();}
