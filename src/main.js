...
import {MIRROR_ARENA,createMirrorFighter,createMirrorPanels,mirrorProjectileSpeed,reflectMirrorPanels,tickMirrorFighter} from './mirror-fighter.js';
...
function mirrorBolt(pos,dir,spec={}){
 ...
 const law=Object.hasOwn(LAWS,spec.law)?spec.law:'seed',start=pos.clone().addScaledVector(dir,1.05),ob=new THREE.Mesh(projectileGeometry(projectileGeos,law),mats['shot-'+law]||mats['shot-seed']),damage=Math.max(5,Math.round(maxPlayerHp()*(spec.damageScale||.075)));
 ob.position.set(start.x,.67,start.z);ob.castShadow=false;ob.rotation.y=Math.atan2(dir.x,dir.z);applyProjectileTheme(ob,law,combatTheme,0,1.08);scene.add(ob);
