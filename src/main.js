...
function drawRoom(){arena=arenaFor(stage,cycle,region);for(const child of [...arenaGroup.children])release(child);buildRoomBoundary();for(const child of [...roomCover.children])release(child);obstacles.splice(0,obstacles.length,...ROOMS[stage].covers.map(o=>({...o})));buildCoverArt(roomCover,obstacles,mats);for(const child of [...trapGroup.children])release(child);traps=trapsFor(stage,cycle);for(const t of traps)t.visual=createTrapVisual(trapGroup,t,mats);}
...
function wave(){
 ...
 for(const [type,x,z] of ROOMS[stage].enemies){if(type==='warden'){const e=createWarden(scene,mats,wardenVariantFor(cycle));e.tint=0xffffff;e.g.position.set(x,0,z);attachActorArt(e,camera,release,{file:`warden-${e.variant}-v4.png`,order:e.variant==='memory'?[0,3,2,1]:[0,1,2,3],size:4.1,directional:true,baseline:.02});enemies.push(e);$('#boss-hud strong').textContent=e.config.name;}else enemy(type,x,z);}
 ...
 for(const spot of turretSpots(stage,cycle)){const t=createTurret(scene,mats,copiedLaws(effectiveLevels(levels,heldForms)));t.g.position.set(spot.x,0,spot.z);attachActorArt(t,camera,release,{file:'enemy-turret-v4.png',size:3.0,preserveBody:true});enemies.push(t);}
...
function showIntro(){pauseBuild.hide();drawRoom();$('#evolution').hidden=true;player.visible=true;paused=false;keys.clear();touch.reset();$('#pause').textContent='Ⅱ';$('#toast').textContent='';$('#boss-hud').hidden=true;$('#exit-room').hidden=true;gate.visible=false;
...
if(mode==='playing'&&!roomCleared&&choiceKills>=killsForChoice(choicesTaken)){choiceKills-=killsForChoice(choicesTaken);choicesTaken++;cardChoice(true);}}
...
function presentFrame(time){
 ...
 const boss=enemies.find(e=>e.type==='warden'&&!e.elite);if(boss){setWidth($('#boss-hud i'),(boss.hp/boss.maxHp*100).toFixed(1)+'%');setText($('#boss-hud small'),pendingEscorts.length?'호위 등장 예고 · 주황 원에서 떨어지세요':boss.variant==='seal'?'보라 원이 닫힐 때 안에 있으면 회피가 봉인됩니다':boss.variant==='hunter'?'돌진 뒤 곧바로 한 번 더 돌진합니다':boss.learned.length?'습득: '+boss.learned.map(id=>LAW_NAMES[id]).join(' · '):'생명 67% · 34%에서 당신의 법칙을 배웁니다');}else setHidden($('#boss-hud'),true);const nearExit=canUseExit({open:exitOpen,mode,paused,x:player.position.x,z:player.position.z});setHidden($('#exit-room'),!nearExit);setText($('#exit-room'),stage===4?'다음 여정 선택 · E':touch.enabled?'다음 방으로':'다음 방으로 · E');gateHalo.rotation.y=time*.6;setWidth($('#hpbar'),hp+'%');setText($('#hptext'),`${hp} / 100`);setWidth($('#dashbar'),((1-dash/2.4)*100).toFixed(1)+'%');setText($('#dashtext'),dashLock>0?'봉인 '+dashLock.toFixed(1)+'s':dash>0?dash.toFixed(1)+'s':'준비');
}
