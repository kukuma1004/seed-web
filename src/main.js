...
import './ranking.css';
import {SLOT_CAP,killsForChoice,levelOf,damageScale,lawStats,offerChoices,chooseLaw,levelsFromSave,levelsToSave,upgradeLine,offeredForm,slotsUsed,fusionLevel,canFuse,fuse,evolveSolo,effectiveLevels,buildLevel,awakenOptions,awakenLevel,awaken} from './progression.js';
...
const sharedGeometries=new Set([...Object.values(enemyGeos),baseOrbitGeo,...Object.values(austinGeo),...Object.values(projectileGeos),...Object.values(ACTOR_ART_GEOMETRIES)]);
...
function wave(){
 ...
 else for(const [type,x,z] of roomFor(stage,cycle).enemies){if(type==='warden'){const e=createWarden(scene,mats,wardenVariantFor(cycle));e.tint=0xffffff;e.g.position.set(x,0,z);attachActorArt(e,camera,release,{file:`warden-${e.variant}-v4.png`,order:e.variant==='memory'?[0,3,2,1]:[0,1,2,3],size:4.1,directional:true,baseline:.02,occlusion:qualityLevel>0});enemies.push(e);$('#boss-hud strong').textContent=e.config.name;}else enemy(type,x,z);}
 if(stage===3){const e=createWarden(scene,mats,wardenVariantFor(cycle+1));e.elite=true;e.tint=0xffffff;e.hp=e.maxHp=1150*.36;{const spot=roomFor(stage,cycle).elite||{x:0,z:-5};e.g.position.set(spot.x,0,spot.z);}attachActorArt(e,camera,release,{file:`warden-${e.variant}-v4.png`,order:e.variant==='memory'?[0,3,2,1]:[0,1,2,3],size:3.1,directional:true,baseline:.02,occlusion:qualityLevel>0});enemies.push(e);}
 for(const spot of inAustinRoom()?[]:turretSpots(stage,cycle)){const t=createTurret(scene,mats,copiedLaws(effectiveLevels(levels,heldForms)));t.g.position.set(spot.x,0,spot.z);attachActorArt(t,camera,release,{file:'enemy-turret-v4.png',size:3.0,preserveBody:true,occlusion:qualityLevel>0});enemies.push(t);}
 if(stage<4&&(stage===1||stage===3||cycle>0)){const e=createShield(scene);{const spot=roomFor(stage,cycle).shield||{x:stage===3?0:1.6,z:stage===3?2:-4};e.g.position.set(spot.x,0,spot.z);}attachActorArt(e,camera,release,{file:'enemy-shield-v4.png',size:2.05,directional:true,baseline:.06,occlusion:qualityLevel>0});enemies.push(e);}
...
function enemyDown(e){
 ...
 if(e.type==='austin'){austinsDefeated++;const firstTitle=!seedTitle.isUnlocked();remember('bosses','austin');const got=austinDrops(rng,inventory).filter(id=>addItem(inventory,id,1)).map(id=>ITEMS[id].name);itemBarKey='';$('#toast').textContent=`${AUSTIN.name} 격파! · ${got.length?got.join(' · ')+' 획득':'물약 가방이 가득 찼습니다'}${firstTitle?` · 칭호 '${AUSTIN_TITLE}' (${AUSTIN_TITLE_PERK.text})`:''}`;}
 else if(main){wardensDefeated++;if(wardensDefeated===1&&!dashState.id)dashRewardPending=true;$('#toast').textContent=`${e.config?.name||'문지기'} 격파!${wardensDefeated===1?' · 뿌리에 새로운 움직임이 깨어납니다':austinAhead()?' · 무언가 째깍거리는 소리가 들립니다':''}`;}
