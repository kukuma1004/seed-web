...
let last=performance.now(),frames=[],frameCounter=0;function animate(now){requestAnimationFrame(animate);let raw=(now-last)/1000;last=now;frames.push(raw*1000);if(frames.length>180)frames.shift();const dt=advanceFrame(raw,now*.001,(step,time)=>{update(step,time);if(!paused)vfx.update(step);});for(let m of motes)m.position.y+=Math.sin(now*.001+m.userData.seed)*dt*.08;look.lerp(new V(player.position.x*.14,0,player.position.z*.06+.4),1-Math.exp(-3.713*dt));camera.position.set(look.x,22,15.5+look.z);cameraShake=Math.max(0,cameraShake-dt);if(cameraShake>0)camera.position.x+=(rng()-.5)*cameraShake;camera.lookAt(look);renderer.info.autoReset=false;renderer.info.reset();composer.render();frameCounter++;if(inspection&&frameCounter%30===0)inspection.textContent=JSON.stringify(window.seedDebug.getState());}
function updateGauges(){
 ...
 const p=player.position.clone().setY(1.95).project(camera),r=renderer.domElement.getBoundingClientRect();
