...
const isBoss=e=>e.type==='warden'||e.type==='austin'||e.type==='act2warden'||e.type==='alwaysbeginner'||e.type==='act3warden'||e.type==='tempestcarrier';
...
function drawRoom(){shadowClock=SHADOW_REFRESH;const clockRoom=inAustinRoom()&&!isAct2(region)&&!isAct3(region),bossRoom=inAustinRoom();arena=clockRoom?AUSTIN_ARENA:arenaFor(stage,cycle,region);for(const child of [...arenaGroup.children])release(child);buildRoomBoundary();clockFloor=clockRoom?createClockFloor(arenaGroup):null;for(const child of [...roomCover.children])release(child);obstacles.splice(0,obstacles.length,...(bossRoom?[]:roomFor(stage,cycle,region).covers).map(o=>({...o})));const exitSpot=arena.exit||EXIT;gate.position.set(exitSpot.x,0,exitSpot.z);if(isAct2(region))buildStadiumCoverArt(roomCover,obstacles,stadium.coverMaterial);else if(!isAct3(region))buildCoverArt(roomCover,obstacles,mats);for(const child of [...trapGroup.children])release(child);traps=bossRoom?[]:trapsFor(stage,cycle,region);createTrapVisuals(trapGroup,traps,mats).forEach((visual,i)=>traps[i].visual=visual);if(!isAct2(region))stadium.setActive(false);if(!isAct3(region))skyway.setActive(false);if(isAct2(region))stadium.setActive(true,arena,{stage,bossRoom});if(isAct3(region))skyway.setActive(true);freezeStatic(arenaGroup);freezeStatic(roomCover);}
...
function wave(){
 ...
 clearForms();cancelActive(activeGauge);activeVfx.clear();finaleEchoes.length=0;cachedTarget=null;targetTimer=0;clearEscorts();escortWaves=0;bossDefeated=false;act2Support=null;bossRewardLine='';vfx.clear();wells.length=0;roomStartKills=kills;midReward=false;trapClock=0;pulls.length=0;orbitHits.clear();dashLock=0;baseSlideTime=baseSlideCooldown=0;baseSlideLock=-1;crowdLeft=inAustinRoom()?0:crowdTotal(stage,cycle)+(isAct3(region)?ACT3_PRESSURE.crowdExtra[stage]||0:isAct2(region)?ACT2_PRESSURE.crowdExtra[stage]||0:0);crowdTimer=0;crowdIndex=0;orbitHitCD=0;
 ...
 drawRoom();{const start=arena.start||{x:0,z:5};player.position.set(start.x,0,start.z);}playerMotion.reset();invuln=Math.max(invuln,.8);$('#boss-hud').classList.toggle('austin-hud',inAustinRoom()&&!isAct2(region)&&!isAct3(region));
 if(inAustinRoom()){
 ...
 for(const e of enemies){e.hp*=(isBoss(e)?difficulty(cycle,region).bossHp:difficulty(cycle,region).hp*(isAct3(region)?ACT3_PRESSURE.hp:isAct2(region)?ACT2_PRESSURE.hp:1))*levelPressure();if(e.maxHp)e.maxHp=e.hp;}
 saveBoundary(stage,inAustinRoom()?'austin':'entry');spawnCrowd(isAct3(region)?ACT3_PRESSURE.crowdInitial:isAct2(region)?ACT2_PRESSURE.crowdInitial:8);combatAnalysis.begin(elapsed,{stage:stage+1,cycle:cycle+1,region});$('#encounter').textContent=`여정 ${cycle+1} · ${inAustinRoom()?(isAct3(region)?'폭풍 중심부 · '+TEMPEST_CARRIER.name:isAct2(region)?'야간 결승전 · '+ALWAYS_BEGINNER.name:'정시의 시계탑 · '+AUSTIN.name):REGION_NAMES[region]+' · '+roomFor(stage,cycle,region).name}`;
...
function showIntro(){trainingSession=null;combatAnalysis.cancel();$('#room-analysis').hidden=true;developerRun=false;labSafe=false;const labButton=$('#developer-lab-fab');if(labButton)labButton.hidden=true;if(gameplayPaused()){showSeasonPause();return;}audio.setScene('garden');region='garden';startRegion='garden';pauseBuild.hide();activeVfx.clear();cancelActive(activeGauge);activeReadyAnnounced=false;$('#active-cinematic').hidden=true;$('#active-cinematic').innerHTML='';austinRoom=false;drawRoom();$('#evolution').hidden=true;player.visible=true;paused=false;keys.clear();touch.reset();$('#pause').textContent='Ⅱ';$('#toast').textContent='';$('#boss-hud').hidden=true;$('#exit-room').hidden=true;gate.visible=false;
...
function presentFrame(time){
 ...
 if(mode!=='playing'||paused)return;
 const wardenGroup=enemies.filter(e=>(e.type==='warden'||e.type==='act2warden'||e.type==='act3warden')&&!e.dead&&(stage===4||!e.elite)),boss=enemies.find(e=>e.type==='austin'||e.type==='alwaysbeginner'||e.type==='tempestcarrier')||wardenGroup.find(e=>e.moveName)||wardenGroup.find(e=>!e.duoSupport&&!e.support)||wardenGroup[0];if(boss)renderBossHud(boss,(boss.type==='austin'||boss.type==='alwaysbeginner'||boss.type==='tempestcarrier')?[boss]:wardenGroup);else setHidden($('#boss-hud'),true);const nearExit=canUseExit({open:exitOpen,mode,paused,x:player.position.x,z:player.position.z,exit:arena.exit||EXIT});setHidden($('#exit-room'),!nearExit);setText($('#exit-room'),stage===4?(finalBossAhead()?'최종 보스에게 · E':'다음 여정으로 · E'):touch.enabled?'다음 방으로':'다음 방으로 · E');gateHalo.rotation.y=time*.6;setWidth($('#hpbar'),(hp/maxPlayerHp()*100).toFixed(1)+'%');setText($('#hptext'),`${displayHp(hp)} / ${displayHp(maxPlayerHp())}`);setText($('#dashname'),`◇  ${dashState.id?DASH_EVOLUTIONS[dashState.id].name:'회피'}`);setWidth($('#dashbar'),(dm.fill*100).toFixed(1)+'%');setText($('#dashtext'),dashLock>0?'봉인 '+dashLock.toFixed(1)+'s':dm.maxCharges>1?`${dm.charges}/${dm.maxCharges}${dm.charges<dm.maxCharges?' · '+dm.recharge.toFixed(1)+'s':''}`:dm.ready?'준비':dm.recharge.toFixed(1)+'s');
}
