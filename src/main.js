...
import {MIRROR_ARENA,MIRROR_FAN,MIRROR_VIEW,createMirrorFighter,createMirrorPanels,mirrorProjectileSpeed,reflectMirrorPanels,tickMirrorFighter} from './mirror-fighter.js';
...
if(autoTarget&&shootCD<=0&&!roomCleared){shootCD=mirrorSession?mirrorAttackCooldown():runCadenceInterval(.22/gardenCooldown(),runBonuses);growth.fire();shotOrigin.copy(player.position).addScaledVector(aimdir,.65);vfx.muzzle(shotOrigin,aimdir,[...chosen][0]||'seed');shotOrigin.copy(player.position).addScaledVector(aimdir,.6);if(mirrorSession){const volley={hit:false};for(const angle of MIRROR_FAN)projectile(shotOrigin,fanDir.copy(aimdir).applyAxisAngle(AXIS_Y,angle)).volley=volley;}else projectile(shotOrigin,aimdir);audio.play('shot');}
