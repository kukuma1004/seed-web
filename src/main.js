...
const masteryRate=id=>mirrorSession?0:((gardenStats()?.points?.[id]||0)*MASTERY_STEP)+codexRate()+(seedTitle?.state().clearStatBonus||0)+(id==='power'?(seedTitle?.state().powerBonus||0):id==='cooldown'?(seedTitle?.state().cooldownBonus||0):0);
let seedTitle=null;
...
function permanentStatsProfile(titleInfo){
 ...
 const row=(id,extra=[])=>{const garden=(pts[id]||0)*MASTERY_STEP,title=id==='move'?titleInfo.moveSpeedBonus||0:id==='power'?titleInfo.powerBonus||0:id==='cooldown'?titleInfo.cooldownBonus||0:0,total=garden+codex+clear+title,parts=[garden?`정원 ${pct(garden)}`:'',codex?`도감 ${pct(codex)}`:'',clear?`완주 ${pct(clear)}`:'',...extra].filter(Boolean);
