...
function syncLaws(){chosen.clear();mutated.clear();const shotLevels=baseShotLevels(levels,heldForms);for(const [id,v] of shotLevels){chosen.add(id);if(v>=2)mutated.add(id);}LS=relicLawStats(lawStats(shotLevels),relics);document.querySelectorAll('#rules>div').forEach(n=>{const lv=levelOf(levels,n.dataset.rule),mark=mutationOf(mutations,n.dataset.rule);n.classList.toggle('active',lv>0);n.classList.toggle('mutated',Boolean(mark));n.querySelector('span:not(.law-art)').textContent=LAWS[n.dataset.rule].name+(lv?' Lv.'+lv:'')+(mark?' '+mark.badge:'');});}
