...
import {MIRROR_ARENA,createMirrorFighter,createMirrorPanels,reflectMirrorPanels,tickMirrorFighter} from './mirror-fighter.js';
...
function mirrorBolt(pos,dir,spec={}){
 ...
 const start=pos.clone().addScaledVector(dir,1.05),ob=enemyBolt(start,'boss',Boolean(spec.frost)),damage=Math.max(5,Math.round(maxPlayerHp()*(spec.damageScale||.075)));
 enemyShots.push({mirror:true,boss:true,ob,dir:dir.clone(),life:5,bounces:enemyBounces(spec.bounces),speed:7.2*(spec.speedScale||1),damage,age:0,origin:pos.clone(),pierce:true,recall:Boolean(spec.recall),frost:Boolean(spec.frost),curve:spec.curve||0,law:spec.law||'pierce'});
}
...
let last=performance.now(),realLast=Date.now(),paceGame=0,paceReal=0,frames=[],frameCounter=0;function animate(now){requestAnimationFrame(animate);let raw=(now-last)/1000;last=now;
 {const realNow=Date.now();const realDelta=(realNow-realLast)/1000;realLast=realNow;if(mode==='playing'&&!paused&&realDelta>0&&realDelta<2){elapsed+=realDelta;paceReal+=realDelta;paceGame+=Math.min(2,Math.max(0,raw));}}if(perfEnabled)try{perf.frame(raw*1000,mode==='playing'&&!paused&&!document.hidden,perfSnapshot);perf.beginFrame();if(perf.due())perf.sampleState(perfSnapshot());}catch{}frames.push(raw*1000);qualityGovernor.sample(raw*1000,mode==='playing'&&!paused&&!document.hidden);if(frames.length>180)frames.shift();const visualDt=Math.min(.1,Math.max(0,raw)),timeScale=cameraFeel.stepEffects(visualDt),dt=advanceFrame(raw*timeScale,now*.001,(step,time)=>update(step,time));audio.tick(visualDt);perfT=performance.now();if(!paused)vfx.update(visualDt);perfMark(PS.particles);frameCounter++;
