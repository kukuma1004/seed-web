...
startMirrorTower:()=>{startMirrorTower({publicRun:false});return true;},hurtBoss:fraction=>{const b=enemies.find(e=>isBoss(e)&&!e.elite&&!e.dead);if(b)damageEnemy(b,b.maxHp*fraction,false);return b?b.hp:null;},givePotion:()=>addItem(inventory,'potion',1),fillActive:()=>{activeGauge.cooldown=0;activeGauge.value=ACTIVE.max;return activeGauge.value;},showAustinTitle:()=>{seedTitle.setUnlocked(true);return seedTitle.isUnlocked();},giveForm:(id,level=4)=>{if(!Object.hasOwn(FORMS,id))return false;heldForms.set(id,level);syncForms();return true;},shotLaws:()=>({laws:[...chosen],mutated:[...mutated],shotPierce,pierceHits:LS.pierceHits,crit:Math.round(totalCritChance()*1000)/1000,eligible:eligibleForms(chosen).map(f=>f.id)}),
