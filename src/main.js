...
function wave(){
 ...
 clearForms();cancelActive(activeGauge);activeVfx.clear();finaleEchoes.length=0;cachedTarget=null;targetTimer=0;clearEscorts();escortWaves=0;bossDefeated=false;act2Support=null;bossRewardLine='';vfx.clear();wells.length=0;roomStartKills=kills;midReward=false;trapClock=0;pulls.length=0;orbitHits.clear();dashLock=0;baseSlideTime=baseSlideCooldown=0;baseSlideLock=-1;crowdLeft=inAustinRoom()?0:crowdTotal(stage,cycle)+(isAct2(region)?ACT2_PRESSURE.crowdExtra[stage]||0:0);crowdTimer=0;crowdIndex=0;orbitHitCD=0;
 ...
 for(const e of enemies){e.hp*=(isBoss(e)?difficulty(cycle,region).bossHp:difficulty(cycle,region).hp*(isAct2(region)?ACT2_PRESSURE.hp:1))*levelPressure();if(e.maxHp)e.maxHp=e.hp;}
 saveBoundary(stage,inAustinRoom()?'austin':'entry');spawnCrowd(isAct2(region)?ACT2_PRESSURE.crowdInitial:8);combatAnalysis.begin(elapsed,{stage:stage+1,cycle:cycle+1,region});$('#encounter').textContent=`여정 ${cycle+1} · ${inAustinRoom()?(isAct2(region)?'야간 결승전 · '+ALWAYS_BEGINNER.name:'정시의 시계탑 · '+AUSTIN.name):REGION_NAMES[region]+' · '+roomFor(stage,cycle,region).name}`;
...
crowdTimer-=dt;if(crowdTimer<=0&&!roomCleared){spawnCrowd(2);crowdTimer=crowdInterval(cycle)*(isAct2(region)?ACT2_PRESSURE.crowdInterval:1);}
enemyIndex.rebuild(enemies);
