...
function useActive(){
 ...
 const colors=plan.forms.flatMap(id=>FORMS[id]?.requires||[]);for(const [i,id] of colors.entries())vfx.pulse(player.position,id,1.5+i*.45,.65,i*.06);
 vfx.burst(player.position,colors[0]||'seed',plan.state==='OVERDRIVE'?60:42,1.8);
...
function overdriveBlast(f,radius,damage,first){
 ...
 vfx.pulse(center,f.slow?'frost':'amber',radius,.75);vfx.pulse(center,'seed',radius*.72,.65,.1);vfx.burst(center,'amber',first?72:30,radius*.58);cameraShake=Math.max(cameraShake,first?.72:.3);
...
if(localInspection&&new URLSearchParams(location.search).has('bossLab')){
 ...
 lab.innerHTML='<span>로컬 합성 검사 · 랭킹 전송 없음</span><button id="lab-hurt">보스 다음 단계</button><button id="lab-heal">검사 생명 회복</button><button id="lab-safe">검사 보호 꺼짐</button><button id="lab-active">오버드라이브 준비</button><button id="lab-solo">단독 진화 외형</button>';
 let soloArtIndex=0;document.body.append(lab);lab.querySelector('#lab-hurt').onclick=()=>window.seedDebug.qa.hurtBoss(1);lab.querySelector('#lab-heal').onclick=()=>window.seedDebug.qa.setHp(100);lab.querySelector('#lab-safe').onclick=()=>{labSafe=!labSafe;lab.querySelector('#lab-safe').textContent=labSafe?'검사 보호 켜짐':'검사 보호 꺼짐';};lab.querySelector('#lab-active').onclick=()=>{window.seedDebug.qa.giveForm('collapse',5);window.seedDebug.qa.giveForm('prism',5);window.seedDebug.qa.fillActive();};lab.querySelector('#lab-solo').onclick=()=>{const id=Object.keys(SOLO_FORMS)[soloArtIndex++%Object.keys(SOLO_FORMS).length];heldForms.clear();heldForms.set(id,5);syncForms();$('#toast').textContent=`외형 검사 · ${FORMS[id].name}`;};
}
