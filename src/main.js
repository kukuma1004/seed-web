...
import {ACT3_REGION,ACT3_NAME,ACT3_PRESSURE,isAct3,act3Unlocked,act3Available,playableAct3Region,act3Storage} from './act3.js';
import {ACT3_GEOMETRIES,ACT3_MATERIALS,isAct3Minion,createAct3Minion,tickAct3Minion,createAct3Warden,tickAct3Warden,TEMPEST_CARRIER,createTempestCarrier,tickTempestCarrier,damageTempestCarrier,act3BossHint,act3BossPatternName} from './act3-enemies.js';
...
function spawnCrowd(n){for(let i=0;i<n&&crowdLeft>0&&enemies.length<CROWD_CAP;i++){const p=safeArenaSpawn(player.position,obstacles,crowdIndex++,arena);if(!p)break;if(isAct3(region))spawnAct3(crowdIndex%4===0?'sky-diver':'sky-scout',p.x,p.z);else swarm(p.x,p.z);crowdLeft--;}}
...
const act3Ctx={get player(){return player.position;},collide:(p,r)=>collide(p,r),hit:a=>{if(invuln>0||shellTime>0)return false;hitPlayer(a);return true;},bolt:(pos,dir,spec)=>skywayBolt(pos,dir,{...spec,speed:spec.speed*difficulty(cycle,region).projectileSpeed*ACT3_PRESSURE.projectile}),sound:id=>audio.play(id)};
function spawnAct3(type,x,z){const e=createAct3Minion(scene,type,rng);e.g.position.set(x,0,z);enemies.push(e);return e;}
function spawnAct3WardenAt(x,z){const e=createAct3Warden(scene);e.g.position.set(x,0,z);enemies.push(e);return e;}
function drawRoom(){
...
function wave(){
 ...
 if(mirrorSession){
  const e=createMirrorFighter(scene,{floor:mirrorSession.floor,quality:qualityLevel===0?'low':'normal',levels,forms:heldForms,dashEvolution:dashState.id});e.g.position.set(0,0,-6.8);enemies.push(e);mirrorSession.enemy=e;mirrorSession.cracks=0;$('#boss-hud strong').textContent=`거울의 탑 ${mirrorSession.floor}층 · 비친 씨앗`;
   ...
   if(isAct3(region)){const e=createTempestCarrier(scene);e.g.position.set(0,0,2.8);enemies.push(e);$('#boss-hud strong').textContent=TEMPEST_CARRIER.name;}
...
function skywayBolt(pos,dir,spec){const ob=new THREE.Mesh(ACT3_GEOMETRIES.bolt,ACT3_MATERIALS.bolt);ob.position.set(pos.x+dir.x*1.05,.72,pos.z+dir.z*1.05);ob.rotation.y=Math.atan2(dir.x,dir.z);ob.castShadow=false;scene.add(ob);enemyShots.push({boss:Boolean(spec.boss),ob,dir:dir.clone(),life:spec.life||4.8,bounces:0,speed:spec.speed,damage:spec.damage,curve:spec.curve||0,age:0,origin:pos.clone(),pierce:Boolean(spec.pierce)});}
// ---------------- active (F) ----------------
function useActive(){
