...
let saveOK=false,profile=readDiscoveries(runStorage);const titleAccount=readAccountProfile(runStorage);seedTitle=createSeedTitle(player,{austin:profile.bosses.includes('austin'),austinClear:profile.bosses.includes('austinclear'),austinVeteran:profile.bosses.includes('austinveteran')||titleAccount.austinWins>=10,alwaysClear:profile.bosses.includes('alwaysclear'),alwaysBeginner:profile.bosses.includes('alwaysbeginner'),discovered:discoveredCount(profile),total:Object.keys(DISCOVERY_FORMS).length,badges:titleAccount.badges,equipped:titleAccount.equippedTitle});
...
async function backfillAustinVeteran(){
 if(localInspection||seedTitle.isAustinVeteranUnlocked()||!account.user()?.uid||account.user().isAnonymous||!online.historicalAustinWins)return;
 try{
  const historic=await online.historicalAustinWins(10),before=readAccountProfile(runStorage),total=Math.max(before.austinWins,historic);
  if(total>before.austinWins)writeAccountProfile(runStorage,{...before,austinWins:total});
  if(total>=10){remember('bosses','austinveteran');void cloud.syncNow();}
 }catch{}
}
