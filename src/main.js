...
const pool=mesh(new THREE.PlaneGeometry(65,65),new THREE.MeshStandardMaterial({color:0x172d35,normalMap:normal,normalScale:new THREE.Vector2(.015,.015),roughness:.68,metalness:.12}),scene,0,-3.4,0);pool.name='garden-pool';pool.rotation.x=-Math.PI/2;
...
if(mode!=='playing'||paused){player.userData.updateArt(camera);return;}const previousPlayer=player.position.clone();potionCD=Math.max(0,potionCD-dt);hasteTime=Math.max(0,hasteTime-dt);shellTime=Math.max(0,shellTime-dt);playerSlow=Math.max(0,playerSlow-dt);baseSlideCooldown=Math.max(0,baseSlideCooldown-dt);tickDash(dashState,dt*gardenCooldown());invuln=Math.max(0,invuln-dt);shootCD-=dt;const move=new V();if(keys.has('KeyW')||keys.has('ArrowUp'))move.z-=1;if(keys.has('KeyS')||keys.has('ArrowDown'))move.z+=1;if(keys.has('KeyA')||keys.has('ArrowLeft'))move.x-=1;if(keys.has('KeyD')||keys.has('ArrowRight'))move.x+=1;move.add(new V(touch.axes.move.x,0,touch.axes.move.y)).clampLength(0,1);if(move.lengthSq()>.001)lastMove.copy(move).normalize();dashLock=Math.max(0,dashLock-dt);const touchDash=touch.consumeDash(),dashRequested=keyboardDash||touchDash;keyboardDash=false;if(dashRequested&&canUseExit({open:exitOpen,mode,paused,x:player.position.x,z:player.position.z,exit:arena.exit||EXIT})){useExit();return;}const dashMove=dashRequested&&dashLock<=0?spendDash(dashState):null;if(dashMove){baseSlideTime=0;runDashes++;invuln=Math.max(invuln,dashMove.invuln);player.userData.dashTime=dashMove.duration;player.userData.dashSpeed=dashMove.speed;player.userData.dashDir=lastMove.clone();vfx.burst(player.position,dashState.id==='molt'?'reflect':'seed',dashState.id==='molt'?22:14);vfx.dash(player.position,player.rotation.y);audio.play('dash');player.userData.dashFX=0;}if(player.userData.dashTime>0){player.userData.dashTime-=dt;player.position.addScaledVector(player.userData.dashDir,dt*(player.userData.dashSpeed||17));player.userData.dashFX+=dt;if(player.userData.dashFX>=.045){player.userData.dashFX=0;vfx.dash(player.position,player.rotation.y);}}else if(baseSlideTime>0){baseSlideTime=Math.max(0,baseSlideTime-dt);player.position.addScaledVector(baseSlideDir,dt*BASE_SLIDE.speed);if(Math.floor(baseSlideTime/.07)!==Math.floor((baseSlideTime+dt)/.07))vfx.dash(player.position,Math.atan2(baseSlideDir.x,baseSlideDir.z));}else{player.position.addScaledVector(move,dt*PLAYER_SPEED*runMoveScale(runBonuses)*(gardenStats()?.move||1)*seedTitle.state().moveSpeed*(playerSlow>0?.75:1)*(hasteTime>0?ITEMS.wind.speed:1));const slide=stadium.tryBaseSlide(player.position,baseSlideCooldown);if(slide){baseSlideDir.set(slide.dx,0,slide.dz);baseSlideTime=slide.duration;baseSlideCooldown=slide.cooldown;lastMove.copy(baseSlideDir);vfx.pulse(player.position,'chain',1.05,.26);audio.play('dash');}}collide(player.position);stadium.tick(dt,player.position,{hit:a=>{if(invuln>0||shellTime>0)return false;hitPlayer(a);return true;},sound:id=>audio.play(id)});aimRing.visible=false;targetTimer-=dt;if(targetTimer<=0||!cachedTarget||cachedTarget.dead){cachedTarget=acquireTarget(player.position,enemies,obstacles,segmentHitsCover);targetTimer=.1;}const autoTarget=cachedTarget;if(autoTarget)aim.copy(autoTarget.g.position);let aimdir=aim.clone().sub(player.position).setY(0).normalize();player.rotation.y=Math.atan2(aimdir.x,aimdir.z);playerMotion.update(dt,player.position.x-previousPlayer.x,player.position.z-previousPlayer.z,{dashing:player.userData.dashTime>0});player.userData.updateArt(camera,player.position.x-previousPlayer.x,player.position.z-previousPlayer.z);player.visible=invuln<.5||Math.floor(time*20)%2===0;player.userData.halo.rotation.z=time*.4;
