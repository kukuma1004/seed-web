...
import {ALL_FORMS as FORMS,FORMS as FIRST_FORMS,SOLO_FORMS,TWIN_FORMS,eligibleForms,formUpgradeLine,soloReady,attackPartsOf,isTwinForm,SECOND_FORMS} from './forms.js';
...
import {codexNews,AUSTIN_CLEAR_TITLE,AUSTIN_CLEAR_MOVE_SPEED,ALWAYS_CLEAR_TITLE,ALWAYS_CLEAR_MAX_HP} from './titles.js';
...
import {BASE_SLIDE,STADIUM_BASES,createStadium} from './stadium.js';
import {RELAY,createRelayRig,isRelayPart,relayName,relayPotionDrop,relayRoom,stopRelay,tickRelayRig} from './act2-relay.js';
import {ACT3_REGION,ACT3_NAME,ACT3_PRESSURE,isAct3,act3Unlocked,act3Available,playableAct3Region,act3Storage,act3CrowdType,act3ReinforcementSpawn} from './act3.js';
...
import {MIRROR_ATTACK_CADENCE,MIRROR_BREAK,MIRROR_TRIAL_PROTOTYPE,mirrorAttackCooldown,mirrorFloorRules,refundMirrorAttackCooldown,readMirrorRecord,recordMirrorResult} from './mirror-trial.js';
import {MIRROR_ARENA,MIRROR_BURST,MIRROR_VIEW,createMirrorFighter,createMirrorPanels,mirrorProjectileSpeed,reflectMirrorPanels,tickMirrorFighter} from './mirror-fighter.js';
import {createProjectileGeometries,projectileGeometry,projectileVisualScale,applyProjectileTheme} from './projectile-art.js';
...
import {readGarden,writeGarden,normalizeGarden,gardenEffects,harvestFromRun,addHarvest,growPlants,harvestLine,activeSlots,centerInfo,SEEDS,grantBossMastery,masteryLine,MASTERY_STEP} from './garden.js';
...
import {createShotBatches,shotObject,addShotParts} from './instanced-shots.js';
...
const shotAuras=createShotAuras(scene,{atlas:shotAtlas,capacity:(MAX_SHOTS+90)*2,mobile:mobileDevice}),auraList=[],shotAurasOn=Boolean(shotAtlas);
...
const sharedGeometries=new Set([...Object.values(ACT2_GEOMETRIES),...Object.values(ACT3_GEOMETRIES),...Object.values(enemyGeos),baseOrbitGeo,stadiumBallGeo,...Object.values(austinGeo),...Object.values(projectileGeos),...Object.values(ACTOR_ART_GEOMETRIES)]);
...
// 내 탄환은 모양·재질이 같은 것끼리 한 번에 그린다(탄환 하나가 드로콜 하나를 쓰지 않게).
const shotBatches=createShotBatches(scene);
// ---------------- 셰이더 미리 준비 ----------------
...
let score=0,wardensDefeated=0,austinsDefeated=0,austinRoom=false,inventory=emptyInventory(),runBonuses=emptyRunBonuses(),runBonusOffer=null,turretPotionDry=0,relics=emptyRelics(),relicRewardPending=false,dashRewardPending=false,clockFloor=null,potionCD=0,act2Support=null,bossRewardLine='';const fallen=[];const JOURNEY_HEAL=25;
const combatAnalysis=createCombatAnalysis();let lastRoomAnalysis=null,trainingSession=null;
const isBoss=e=>e.type==='warden'||e.type==='austin'||e.type==='act2warden'||e.type==='alwaysbeginner'||e.type==='act3warden'||e.type==='tempestcarrier'||e.type==='mirrorseed';
...
// 찐보스를 열 번째로 이긴 순간인가. (옛 저장처럼 이미 열 번을 넘겼다면 다음 찐보스에서 끝난다.)
...
function applyLawHit(e,amount,secondary=false,critical=false){
 ...
 if(chosen.has('frost')){e.slow=Math.max(e.slow||0,LS.frostTime);combatAnalysis.utility('freeze');}
 if(secondary)return;
 if(chosen.has('burst')){
...
const skyway=createSkyway(scene,{lights:worldLights,hide:hiddenGarden,mobile:mobileDevice});
...
const act3Ctx={get player(){return player.position;},collide:(p,r)=>collide(p,r),hit:a=>{if(invuln>0||shellTime>0)return false;hitPlayer(a);return true;},bolt:(pos,dir,spec)=>skywayBolt(pos,dir,{...spec,speed:spec.speed*difficulty(cycle,region).projectileSpeed*ACT3_PRESSURE.projectile}),sound:id=>audio.play(id),summon:types=>{for(const type of types){const p=act3ReinforcementSpawn(crowdIndex++),minion=spawnAct3(type,p.x,p.z);minion.hp*=difficulty(cycle,region).hp*ACT3_PRESSURE.hp*levelPressure();minion.maxHp=minion.hp;vfx.pulse(minion.g.position,'chain',1.25,.3);}}};
...
function drawRoom(){
 ...
 for(const child of [...roomCover.children])release(child);obstacles.splice(0,obstacles.length,...(bossRoom?[]:roomFor(stage,cycle,region).covers).map(o=>({...o})));
 const exitSpot=arena.exit||EXIT;gate.position.set(exitSpot.x,0,exitSpot.z);if(!mirrorRoom&&isAct2(region))buildStadiumCoverArt(roomCover,obstacles,stadium.coverMaterial);else if(!mirrorRoom&&!isAct3(region))buildCoverArt(roomCover,obstacles,mats);
...
function finishChoice(){runBonusOffer=null;mode='playing';$('#overlay').hidden=true;invuln=Math.max(invuln,.7);shootCD=0;if(roomCleared){if(mirrorSession)advanceMirrorFloor();else openExit();}}
...
function formHit(e,amount,meta){
 ...
 const scale=damageScale(levels)*relicFormScale(relics,consumed)*runPowerScale(runBonuses)*gardenPower()*(critical?LS.critDamage:1);damageEnemy(e,amount*scale,false,critical,meta);twinResonance(e,amount,meta,scale);
...
function enterAustin(){austinRoom=true;wave();$('#toast').textContent=isAct3(region)?`${TEMPEST_CARRIER.name} 등장 · 발사구 섬광과 날개 움직임으로 탄막의 틈을 찾으세요`:isAct2(region)?`${ALWAYS_BEGINNER.name} 등장 · 투구선·베이스·부채꼴을 읽고 끝까지 버티세요`:`${AUSTIN.name} 등장 · 바닥 시계의 침이 다음 종소리의 빈틈을 가리킵니다`;}
...
function wave(){
 ...
 clearForms();cancelActive(activeGauge);activeVfx.clear();finaleEchoes.length=0;cachedTarget=null;targetTimer=0;clearEscorts();escortWaves=0;bossDefeated=false;act2Support=null;bossRewardLine='';vfx.clear();wells.length=0;roomStartKills=kills;midReward=false;trapClock=0;pulls.length=0;orbitHits.clear();dashLock=0;baseSlideTime=baseSlideCooldown=0;baseSlideLock=-1;crowdLeft=mirrorSession||inAustinRoom()?0:crowdTotal(stage,cycle)+(isAct3(region)?ACT3_PRESSURE.crowdExtra[stage]||0:isAct2(region)?ACT2_PRESSURE.crowdExtra[stage]||0:0);crowdTimer=0;crowdIndex=0;orbitHitCD=0;
 ...
 drawRoom();{const start=arena.start||{x:0,z:5};player.position.set(start.x,0,start.z);}playerMotion.reset();invuln=Math.max(invuln,mirrorSession?1.15:.8);$('#boss-hud').classList.toggle('austin-hud',!mirrorSession&&inAustinRoom()&&!isAct2(region)&&!isAct3(region));
 if(mirrorSession){
 ...
 if(!mirrorSession&&isAct2(region)&&relayRoom(stage,inAustinRoom())){const rig=createRelayRig(scene);enemies.push(rig.machine,rig.mitt);}
...
function enemyBolt(pos,kind,frost=false){
 ...
 // 불투명한 핵과 가시는 같은 종류끼리 한 번에 그리고(탄 하나가 최대 여섯 번 그리던 것), 반투명 고리만 따로 둔다.
 const core=shotObject(boss?enemyGeos.bossCore:enemyGeos.boltCore,frost?mats.frostBolt:boss?mats.bossBolt:mats.enemyBolt),parts=[core];
 ...
 if(boss)for(let i=0;i<4;i++){const out=new V(Math.cos(i*Math.PI/2),0,Math.sin(i*Math.PI/2));const spike=shotObject(enemyGeos.bossSpike,mats.bossBolt);spike.quaternion.setFromUnitVectors(new V(0,1,0),out);spike.position.copy(out).multiplyScalar(.36);parts.push(spike);}
 return addShotParts(g,parts);
}
// 3막은 하늘길이 넓어 화면 밖에서 적과 탄이 날아온다. 그 막에서만 카메라를 뒤로 빼 한눈에 보이게 한다.
const ACT3_VIEW_ZOOM=.82;
...
function mirrorBolt(pos,dir,spec={}){
 ...
 const law=Object.hasOwn(LAWS,spec.law)?spec.law:'seed',start=pos.clone().addScaledVector(dir,1.05),ob=shotObject(projectileGeometry(projectileGeos,law),mats['shot-'+law]||mats['shot-seed']),damage=Math.max(5,Math.round(maxPlayerHp()*(spec.damageScale||.075)));
...
function mirrorPerfectDodge(projectile){
 ...
 const enemy=mirrorSession.enemy;if(mirrorSession.cracks>=MIRROR_BREAK.crackGoal&&enemy&&!enemy.dead){mirrorSession.cracks=0;enemy.broken=MIRROR_BREAK.breakDuration;enemy.cracks=0;shootCD=MIRROR_ATTACK_CADENCE.breakInstantReady?0:shootCD;chargeActive(activeGauge,ACTIVE.max*MIRROR_BREAK.ultimateCharge);vfx.pulse(enemy.g.position,'amber',2.4,.5);vfx.burst(enemy.g.position,'amber',28,1.6);cameraShake=Math.max(cameraShake,.3);$('#toast').textContent=`거울 깨짐 · ${MIRROR_BREAK.breakDuration.toFixed(1)}초 동안 피해 ${Math.round((MIRROR_BREAK.damageMultiplier-1)*100)}% 증가`;}
 else $('#toast').textContent=`완벽 회피 · 거울 균열 ${mirrorSession.cracks}/${MIRROR_BREAK.crackGoal} · 다음 자동공격이 빨라집니다`;
...
function damageEnemy(e,amount,chaining=true,critical=false,meta=null){
 ...
 if(e.hp<=0&&!e.dead){e.dead=true;kills++;choiceKills++;combatAnalysis.kill();chargeActive(activeGauge,killCharge(e));vfx.impact(e.g.position,'amber',true);enemyDown(e);}
}
...
function austinBolt(pos,dir,spec){const g=new THREE.Group();g.position.set(pos.x+dir.x*1.3,.65,pos.z+dir.z*1.3);scene.add(g);addShotParts(g,[shotObject(austinGeo.core,mats.austinBolt)]);const halo=new THREE.Mesh(austinGeo.halo,mats.austinHalo);halo.rotation.x=-Math.PI/2;halo.castShadow=false;g.add(halo);enemyShots.push({boss:true,ob:g,dir:dir.clone(),life:4,bounces:0,speed:spec.speed,damage:spec.damage,age:0,origin:pos.clone(),pierce:true});}
function stadiumBolt(pos,dir,spec){const ob=shotObject(stadiumBallGeo,mats.stadiumBall);ob.position.set(pos.x+dir.x*1.05,.65,pos.z+dir.z*1.05);enemyShots.push({boss:Boolean(spec.boss),ob,dir:dir.clone(),life:spec.life||4.8,bounces:enemyBounces(spec.bounces),speed:spec.speed,damage:spec.damage,curve:spec.curve||0,age:0,origin:pos.clone(),pierce:Boolean(spec.pierce)});}
function skywayBolt(pos,dir,spec){const cap=qualityLevel===0?ACT3_PRESSURE.projectileCapLow:ACT3_PRESSURE.projectileCapNormal;if(enemyShots.length>=cap)return;const boss=Boolean(spec.boss),ob=new THREE.Mesh(boss?ACT3_GEOMETRIES.bossBolt:ACT3_GEOMETRIES.bolt,boss?ACT3_MATERIALS.bossBolt:ACT3_MATERIALS.bolt);ob.position.set(pos.x+dir.x*1.05,.72,pos.z+dir.z*1.05);ob.rotation.y=Math.atan2(dir.x,dir.z);ob.scale.setScalar(spec.scale||1);ob.castShadow=false;scene.add(ob);enemyShots.push({boss,ob,dir:dir.clone(),life:spec.life||4.8,bounces:0,speed:spec.speed,damage:spec.damage,curve:spec.curve||0,age:0,origin:pos.clone(),pierce:Boolean(spec.pierce)});}
// ---------------- active (F) ----------------
function useActive(){
...
function projectile(pos,dir,fragment=false,ignoreEnemy=null){const tint=fragment?'split':([...chosen][0]||'seed'),critChance=totalCritChance(),critical=critChance>0&&rng()<critChance,k=projectileVisualScale(fragment,critical),ob=shotObject(projectileGeometry(projectileGeos,tint),critical?mats.criticalShot:mats['shot-'+tint]);ob.position.set(pos.x,.67,pos.z);ob.rotation.y=Math.atan2(dir.x,dir.z);applyProjectileTheme(ob,tint,combatTheme,0,k);const shot={ob,dir:dir.clone(),life:2.3,bounces:0,fragment,ignoreEnemy,tint,critical,visualScale:k,age:0,returning:false,hitSet:new Set(),hits:0,trailTime:0,trailPos:ob.position.clone()};shots.push(shot);perf.event(PE.shot);return shot;}
...
function cardChoice(mid=false,fixedOffer=null){
 ...
 if(runBonusOffer===null)runBonusOffer=mirrorSession?[]:rareRunBonusOffers(runBonuses,{hp,maxHp:maxPlayerHp(),choicesTaken,random:rng});
 const smallGrowth=runBonusOffer;
 $('#overlay').hidden=false;$('#overlay').innerHTML=`<p>${mirrorSession?`거울의 탑 ${mirrorSession.floor}층 돌파 · 분신도 같은 선택을 얻습니다`:mid?'처치 게이지 가득':inAustinRoom()?'오스틴 격파 보상':'문지기 격파 보상'} · 슬롯 ${slotsUsed(levels,heldForms)}/${SLOT_CAP}</p><h2>${offered.some(id=>offeredFusion(id))?'합칠까, 더 깊게 갈까'
...
function showMirrorTrialResult(won,report=null){
 const floor=mirrorSession?.floor||1,perfect=mirrorSession?.perfectDodges||0,publicRun=Boolean(mirrorSession?.publicRun),record=publicRun?recordMirrorResult(runStorage,{floor,won,perfectDodges:perfect}):readMirrorRecord(runStorage),shownFloor=publicRun?(record.bestFloor||floor):floor;mode='mirror-result';paused=false;touch.reset();keys.clear();mirrorReadyRing.visible=false;for(const p of enemyShots)release(p.ob);enemyShots=[];$('#boss-hud').hidden=true;$('#overlay').hidden=false;$('#overlay').classList.add('intro','menu-screen');$('#overlay').classList.toggle('developer-mode',!publicRun);
 $('#overlay').innerHTML=`<div class="menu-panel developer-panel"><p class="eyebrow">SEED · MIRROR TOWER</p><h2>${won?'거울의 탑 10층 돌파':'거울에 잠든 씨앗'}</h2><p>${won?'같은 선택을 나눈 마지막 분신을 이겼습니다.':`${floor}층에서 시련이 끝났습니다.`}</p><div class="final-score"><small>${publicRun?'나의 최고 기록':'개발자 검사 기록'}</small><strong>${shownFloor}층</strong><span>이번 도전 완벽 회피 ${perfect}회${publicRun&&record.clears?` · 완주 ${record.clears}회`:''}</span></div>${report?combatAnalysisSummary(report):''}<div class="developer-footer"><button class="primary" id="mirror-retry">1층부터 다시</button><button id="mirror-end">던전으로</button></div></div>`;
 $('#mirror-retry').onclick=()=>startMirrorTower({publicRun});$('#mirror-end').onclick=showDungeon;
}
function advanceMirrorFloor(){
 ...
 const rules=mirrorFloorRules(mirrorSession.floor),heal=Math.round(maxPlayerHp()*rules.healAfter);mirrorSession.floor++;mirrorSession.cracks=0;hp=Math.min(maxPlayerHp(),hp+heal);roomCleared=false;invuln=1;shootCD=0;wave();$('#toast').textContent=`${mirrorSession.floor}층 · 생명력 +${heal} · 분신도 방금 선택을 얻었습니다`;
}
function startMirrorTower({publicRun=true}={}){
 ...
 developerRun=true;labSafe=false;startRegion='garden';mirrorSession={floor:1,cracks:0,perfectDodges:0,enemy:null,publicRun};restart();
}
...
function showDungeon(){
 ...
 const mirrorReady=MIRROR_TRIAL_PROTOTYPE.released&&austinKnown(),mirrorRecord=readMirrorRecord(runStorage);
 ...
 $('#overlay').innerHTML=`<div class="menu-panel dungeon-panel">
  ...
  <div class="menu-list">
   ...
   ${mirrorReady?`<button id="start-mirror" class="menu-item mirror-button"><strong>거울의 탑 · 10층</strong><small>${mirrorRecord.bestFloor?`최고 ${mirrorRecord.bestFloor}층 · `:''}같은 선택을 얻는 분신과 정면 승부</small></button>`:'<p class="act2-lock">오스틴을 쓰러뜨리면 별도 도전 · 거울의 탑이 열려요</p>'}
...
function restart(saved=null){if(gameplayPaused()){showSeasonPause();return;}if(maintenanceOn){showMaintenance();return;}const candidate=saved?.version===1?saved:null,playable=r=>playableAct3Region(playableRegion(r),globalThis.location,developerRun),regionBlocked=candidate&&playable(candidate.region)!==candidate.region,restore=regionBlocked?null:candidate;region=playable(restore?.region||startRegion);if(touch.enabled)appShell.enterFullscreen();perfBegin();if(!restore&&!developerRun)clearCheckpoint(actStore());heldForms.clear();rerollUsed=restore?.rerollUsed===true;if(restore)guideTarget=profile.forms.includes(restore.guideTarget)?restore.guideTarget:null;promptedForms.clear();clearEscorts();vfx.clear();wells.length=0;orbitGroup.visible=false;touch.reset();for(let e of enemies)releaseEnemy(e);for(const f of fallen)releaseEnemy(f.e);fallen.length=0;for(let p of [...shots,...enemyShots,...effects])release(p.ob);enemies=[];shots=[];enemyShots=[];effects=[];levels.clear();syncLaws();choicesTaken=0;choiceKills=0;runBonusOffer=null;dashLock=0;pulls.length=0;orbitHits.clear();chosen.clear();mutated.clear();roomCleared=false;exitOpen=false;growth.reset();playerMotion.reset();player.visible=true;evolutionTime=0;$('#evolution').hidden=true;$('#active-cinematic').hidden=true;$('#active-cinematic').innerHTML='';updateFormLabel();document.querySelectorAll('#rules>div').forEach(n=>n.classList.remove('active'));hp=maxPlayerHp();playerSlow=0;dashState=createDashState();invuln=1;shootCD=0;keyboardDash=false;keys.clear();player.userData.dashTime=0;stage=0;kills=0;elapsed=0;runDashes=0;runDamageTaken=0;player.position.set(0,0,5);mode='playing';paused=false;$('#overlay').hidden=true;$('#pause').textContent='Ⅱ';$('#toast').textContent='';$('#overlay').classList.remove('intro');lastMove.set(0,0,1);cycle=restore?.cycle||0;score=restore?restoredScore(restore):0;paceGame=0;paceReal=0;wardensDefeated=restore?restoredWardens(restore):0;austinsDefeated=restore?restoredAustins(restore):0;inventory=restore?normalizeInventory(restore.inventory):startingInventory();runBonuses=normalizeRunBonuses(restore?.runBonuses);if(!restore&&!developerRun)for(const [id,n] of Object.entries(claimCarry(runStorage)))addItem(inventory,id,n);turretPotionDry=restore?.turretPotionDry||0;hasteTime=0;shellTime=0;selectedItem=null;itemBarKey='';relics=normalizeRelics(restore?.relics);relicRewardPending=false;dashState=createDashState(restore?.dashEvolution);dashRewardPending=Boolean(restore&&wardensDefeated>=1&&!dashState.id);austinRoom=false;potionCD=0;
 ...
 if(!restore){
  ...
  $('#toast').textContent=mirrorSession?'거울의 탑 · 물약·유물·정원·칭호 없이 같은 선택을 한 분신과 싸웁니다':departure
...
perfMark(PS.animation);if(mode!=='playing'||paused){cameraMoveX=0;cameraMoveZ=0;mirrorReadyRing.visible=false;player.userData.updateArt(camera);return;}if(trainingSession&&elapsed-trainingSession.start>=trainingSession.duration){finishTraining();return;}previousPlayer.copy(player.position);potionCD=Math.max(0,potionCD-dt);hasteTime=Math.max(0,hasteTime-dt);shellTime=Math.max(0,shellTime-dt);playerSlow=Math.max(0,playerSlow-dt);baseSlideCooldown=Math.max(0,baseSlideCooldown-dt);tickDash(dashState,dt*gardenCooldown());invuln=Math.max(0,invuln-dt);shootCD-=dt;const titleState=mirrorSession?{moveSpeed:1,shotSpeed:1}:seedTitle.state();const move=moveVector.set(0,0,0);if(keys.has('KeyW')||keys.has('ArrowUp'))move.z-=1;if(keys.has('KeyS')||keys.has('ArrowDown'))move.z+=1;if(keys.has('KeyA')||keys.has('ArrowLeft'))move.x-=1;if(keys.has('KeyD')||keys.has('ArrowRight'))move.x+=1;move.x+=touch.axes.move.x;move.z+=touch.axes.move.y;move.clampLength(0,1);if(move.lengthSq()>.001)lastMove.copy(move).normalize();dashLock=Math.max(0,dashLock-dt);const touchDash=touch.consumeDash(),dashRequested=keyboardDash||touchDash;keyboardDash=false;if(dashRequested&&canUseExit({open:exitOpen,mode,paused,x:player.position.x,z:player.position.z,exit:arena.exit||EXIT})){useExit();return;}const dashMove=dashRequested&&dashLock<=0?spendDash(dashState):null;if(dashMove){baseSlideTime=0;runDashes++;invuln=Math.max(invuln,dashMove.invuln);player.userData.dashTime=dashMove.duration;player.userData.dashSpeed=dashMove.speed;player.userData.dashDir=lastMove.clone();vfx.burst(player.position,dashState.id==='molt'?'reflect':'seed',dashState.id==='molt'?22:14);vfx.dash(player.position,player.rotation.y);audio.play('dash');player.userData.dashFX=0;}let sliding=false;if(player.userData.dashTime>0){player.userData.dashTime-=dt;player.position.addScaledVector(player.userData.dashDir,dt*(player.userData.dashSpeed||17));player.userData.dashFX+=dt;if(player.userData.dashFX>=.045){player.userData.dashFX=0;vfx.dash(player.position,player.rotation.y);}}else if(baseSlideTime>0){sliding=true;const remaining=baseSlideRemaining.copy(baseSlideTarget).sub(player.position).setY(0),distance=remaining.length(),step=Math.min(distance,dt*BASE_SLIDE.speed);baseSlideTime=Math.max(0,baseSlideTime-dt);if(distance>.001){baseSlideDir.copy(remaining).normalize();player.position.addScaledVector(baseSlideDir,step);}if(distance<=step+.001||baseSlideTime<=0){player.position.x=baseSlideTarget.x;player.position.z=baseSlideTarget.z;baseSlideTime=0;}if(Math.floor(baseSlideTime/.07)!==Math.floor((baseSlideTime+dt)/.07))vfx.dash(player.position,Math.atan2(baseSlideDir.x,baseSlideDir.z));}else{player.position.addScaledVector(move,dt*PLAYER_SPEED*runMoveScale(runBonuses)*(1+masteryRate('move'))*titleState.moveSpeed*(playerSlow>0?.75:1)*(hasteTime>0?ITEMS.wind.speed:1));const occupiedBase=stadium.baseAt(player.position);if(baseSlideLock>=0&&occupiedBase!==baseSlideLock)baseSlideLock=-1;const slide=stadium.tryBaseSlide(player.position,baseSlideCooldown,baseSlideLock);if(slide){baseSlideDir.set(slide.dx,0,slide.dz);baseSlideTarget.set(slide.targetX,0,slide.targetZ);baseSlideTime=slide.duration;baseSlideCooldown=slide.cooldown;baseSlideLock=STADIUM_BASES[slide.index].next;lastMove.copy(baseSlideDir);vfx.pulse(player.position,'chain',1.05,.26);audio.play('dash');}}if(sliding)constrainToArena(player.position,.4,arena);else collide(player.position);aimRing.visible=false;targetTimer-=dt;if(targetTimer<=0||!cachedTarget||cachedTarget.dead){cachedTarget=acquireTarget(player.position,enemies,obstacles,segmentHitsCover);targetTimer=.1;}const autoTarget=cachedTarget;if(autoTarget)aim.copy(autoTarget.g.position);const aimdir=aimDirection.copy(aim).sub(player.position).setY(0).normalize();player.rotation.y=Math.atan2(aimdir.x,aimdir.z);cameraMoveX=player.position.x-previousPlayer.x;cameraMoveZ=player.position.z-previousPlayer.z;playerMotion.update(dt,player.position.x-previousPlayer.x,player.position.z-previousPlayer.z,{dashing:player.userData.dashTime>0||sliding});player.userData.updateArt(camera,player.position.x-previousPlayer.x,player.position.z-previousPlayer.z);player.visible=invuln<.5||Math.floor(time*20)%2===0;player.userData.halo.rotation.z=time*.4;
...
perfMark(PS.enemyAI);crowdTimer-=dt;if(crowdTimer<=0&&!roomCleared){spawnCrowd(2);crowdTimer=crowdInterval(cycle)*(isAct3(region)?ACT3_PRESSURE.crowdInterval:isAct2(region)?ACT2_PRESSURE.crowdInterval:1);}
...
let last=performance.now(),realLast=Date.now(),paceGame=0,paceReal=0,frames=[],frameCounter=0;function animate(now){requestAnimationFrame(animate);let raw=(now-last)/1000;last=now;
 {const realNow=Date.now();const realDelta=(realNow-realLast)/1000;realLast=realNow;if(mode==='playing'&&!paused&&realDelta>0&&realDelta<2){elapsed+=realDelta;paceReal+=realDelta;paceGame+=Math.min(2,Math.max(0,raw));}}if(perfEnabled)try{perf.frame(raw*1000,mode==='playing'&&!paused&&!document.hidden,perfSnapshot);perf.beginFrame();if(perf.due())perf.sampleState(perfSnapshot());}catch{}frames.push(raw*1000);if(frames.length>180)frames.shift();const visualDt=Math.min(.1,Math.max(0,raw)),timeScale=cameraFeel.stepEffects(visualDt),dt=advanceFrame(raw*timeScale,now*.001,(step,time)=>update(step,time));audio.tick(visualDt);perfT=performance.now();if(!paused)vfx.update(visualDt);perfMark(PS.particles);frameCounter++;
...
function updateGauges(){
 ...
 setText($('#growth-progress span'),mirrorSession?`완벽 회피로 균열 ${mirrorSession.cracks}/${MIRROR_BREAK.crackGoal} · 승리하면 같은 법칙 선택`:`다음 법칙까지 ${need-have} 처치 · 슬롯 ${slotsUsed(levels,heldForms)}/${SLOT_CAP}`);setWidth($('#growth-progress i'),mirrorSession?(mirrorSession.cracks/MIRROR_BREAK.crackGoal*100).toFixed(1)+'%':(have/need*100).toFixed(1)+'%');
 if(mode!=='playing'||paused)return;
...
function applyCombatTheme(id,{save=true}={}){
 ...
 for(const law of ['seed',...Object.keys(LAWS)])mats['shot-'+law].color.setHex(themeColor(id,`shot:${law}`,FX_COLORS[law])).multiplyScalar(THEMES[id].projectileGlow);
...
function perfFinishNow(outcome){
 ...
 const report=perf.end({outcome,context:{act:isAct3(region)?3:isAct2(region)?2:1,journey:cycle+1,room:stage,test:developerRun,mirror:Boolean(mirrorSession),training:Boolean(trainingSession),build:buildRecord({levels,forms:heldForms,relic:relics.equipped,wardens:wardensDefeated,austins:austinsDefeated})}});
...
window.addEventListener('pagehide',()=>perfFinish('closed'));
