...
const DIRECT_FORMS=new Set(['returnblade','prism','thunderlance','seedstorm','mirrorguard','gravitymirror','blastlance','frostkaleidoscope','lightningpetal','returnflare','mirrormaze','fullbloom','glassspear','rewind','icicle','refractlance','halobloom']);
...
import {codexNews,AUSTIN_CLEAR_TITLE,AUSTIN_CLEAR_MOVE_SPEED} from './titles.js';
...
import {killPoints,roomPoints,submitScore,readRanking,lastName,saveName,cleanName,escapeHtml,rankingTable,formatScore,NAME_MAX,formatTime} from './score.js';
import {createOnlineRanking,SEASON,PREVIOUS_SEASON,ARCHIVE_SEASON,ACT,runAct} from './online-ranking.js';
...
function completeRun(){
 ...
 // 1막을 완주하면 칭호 '정시를 정복한 자'(이동 속도 +10%). 실험 판에는 주지 않는다.
 const firstClear=!developerRun&&!isAct2(region)&&!isAct3(region)&&!seedTitle.isAustinClearUnlocked();
 if(!developerRun&&!isAct2(region)&&!isAct3(region))remember('bosses','austinclear');
 showEnd(null,{cleared:true,newTitle:firstClear?AUSTIN_CLEAR_TITLE:null});
}
...
let saveOK=false,profile=readDiscoveries(runStorage);const titleAccount=readAccountProfile(runStorage);seedTitle=createSeedTitle(player,{austin:profile.bosses.includes('austin'),austinClear:profile.bosses.includes('austinclear'),alwaysBeginner:profile.bosses.includes('alwaysbeginner'),discovered:profile.forms.length,total:Object.keys(FORMS).length,badges:titleAccount.badges,equipped:titleAccount.equippedTitle});
...
function stadiumBolt(pos,dir,spec){const ob=new THREE.Mesh(stadiumBallGeo,mats.stadiumBall);ob.position.set(pos.x+dir.x*1.05,.65,pos.z+dir.z*1.05);ob.castShadow=false;scene.add(ob);enemyShots.push({boss:Boolean(spec.boss),ob,dir:dir.clone(),life:spec.life||4.8,bounces:enemyBounces(spec.bounces),speed:spec.speed,damage:spec.damage,curve:spec.curve||0,age:0,origin:pos.clone(),pierce:Boolean(spec.pierce)});}
...
function projectile(pos,dir,fragment=false,ignoreEnemy=null){const tint=fragment?'split':([...chosen][0]||'seed'),critChance=totalCritChance(),critical=critChance>0&&rng()<critChance,k=projectileVisualScale(fragment,critical),ob=new THREE.Mesh(projectileGeometry(projectileGeos,tint),critical?mats.criticalShot:mats['shot-'+tint]);ob.position.set(pos.x,.67,pos.z);ob.castShadow=false;ob.rotation.y=Math.atan2(dir.x,dir.z);applyProjectileTheme(ob,tint,combatTheme,0,k);scene.add(ob);const shot={ob,dir:dir.clone(),life:2.3,bounces:0,fragment,ignoreEnemy,tint,critical,visualScale:k,age:0,returning:false,hitSet:new Set(),hits:0,trailTime:0,trailPos:ob.position.clone()};shots.push(shot);return shot;}
...
function showEnd(deathReport=null,{cleared=false,newTitle=null}={}){touch.reset();$('#overlay').classList.remove('intro','menu-screen','garden-mode');activeVfx.clear();cancelActive(activeGauge);$('#active-cinematic').hidden=true;$('#active-cinematic').innerHTML='';$('#item-bar').hidden=true;$('#active-skill').hidden=true;$('#item-status').hidden=true;$('#room-analysis').hidden=true;
 ...
 if(newTitle)$('#rank-status').insertAdjacentHTML('beforebegin',`<p class="clear-title">새 칭호 <b>'${escapeHtml(newTitle)}'</b> · 이동 속도 +${Math.round(AUSTIN_CLEAR_MOVE_SPEED*100)}%</p>`);
 $('#restart').onclick=showIntro;
 $('#end-ranking').onclick=()=>showRanking('online');
 if(!ranked)return;
 if(!betaRankingEligible()){setText($('#rank-status'),`이 기기 기록에는 남았어요 · ${SEASON.name.split(' · ')[0]} 랭킹은 Android 앱 또는 등록된 PC 웹 테스터의 Google 계정 기록만 받아요`);return;}
...
crowdTimer-=dt;if(crowdTimer<=0&&!roomCleared){spawnCrowd(2);crowdTimer=crowdInterval(cycle)*(isAct3(region)?ACT3_PRESSURE.crowdInterval:isAct2(region)?ACT2_PRESSURE.crowdInterval:1);}
enemyIndex.rebuild(enemies);
...
if(!mirrorSession&&mode==='playing'&&!roomCleared&&!bossFalling&&choiceKills>=killsForChoice(choicesTaken)){choiceKills-=killsForChoice(choicesTaken);choicesTaken++;cardChoice(true);}}
