...
let score=0,wardensDefeated=0,austinsDefeated=0,austinRoom=false,inventory=emptyInventory(),runBonuses=emptyRunBonuses(),runBonusOffer=null,turretPotionDry=0,relics=emptyRelics(),relicRewardPending=false,dashRewardPending=false,clockFloor=null,potionCD=0;const fallen=[];const JOURNEY_HEAL=25;
...
const act2Ctx={get player(){return player.position;},collide:(p,r)=>collide(p,r),hit:a=>hitPlayer(a),
 bolt:(pos,dir,spec)=>{const ob=enemyBolt(pos,'caster');enemyShots.push({ob,dir:dir.clone(),life:4,speed:spec.speed*difficulty(cycle,region).projectileSpeed,damage:spec.damage,curve:spec.curve||0});},
...
function formHit(e,amount,meta){
 ...
 const scale=damageScale(levels)*relicFormScale(relics,consumed)*runPowerScale(runBonuses)*gardenPower();damageEnemy(e,amount*scale,false);twinResonance(e,amount,meta,scale);
...
function saveAfterBoss(){if(austinAhead())return saveBoundary(4,'austin');return saveBoundary(0,'entry',{cycle:cycle+1,hp:Math.min(maxPlayerHp(),hp+JOURNEY_HEAL),rerollUsed:false});}
...
function wave(){
 ...
 clearForms();cancelActive(activeGauge);activeVfx.clear();finaleEchoes.length=0;cachedTarget=null;targetTimer=0;clearEscorts();escortWaves=0;bossDefeated=false;vfx.clear();wells.length=0;roomStartKills=kills;midReward=false;trapClock=0;pulls.length=0;orbitHits.clear();dashLock=0;crowdLeft=crowdTotal(stage,cycle);crowdTimer=0;crowdIndex=0;orbitHitCD=0;
...
function enemyDown(e){
 ...
 const stageWarden=e.type==='warden'&&stage===4&&!inAustinRoom(),lastWarden=stageWarden&&!enemies.some(o=>o!==e&&o.type==='warden'&&!o.dead&&!o.escort);
 ...
 const main=e.type==='austin'||(stageWarden?lastWarden:!e.elite);fallen.push({e,t:0,hold:main});
...
if(mode!=='playing'||paused){player.userData.updateArt(camera);return;}const previousPlayer=player.position.clone();potionCD=Math.max(0,potionCD-dt);hasteTime=Math.max(0,hasteTime-dt);shellTime=Math.max(0,shellTime-dt);playerSlow=Math.max(0,playerSlow-dt);tickDash(dashState,dt*gardenCooldown());invuln=Math.max(0,invuln-dt);shootCD-=dt;const move=new V();if(keys.has('KeyW')||keys.has('ArrowUp'))move.z-=1;if(keys.has('KeyS')||keys.has('ArrowDown'))move.z+=1;if(keys.has('KeyA')||keys.has('ArrowLeft'))move.x-=1;if(keys.has('KeyD')||keys.has('ArrowRight'))move.x+=1;move.add(new V(touch.axes.move.x,0,touch.axes.move.y)).clampLength(0,1);if(move.lengthSq()>.001)lastMove.copy(move).normalize();dashLock=Math.max(0,dashLock-dt);const touchDash=touch.consumeDash(),dashRequested=keyboardDash||touchDash;keyboardDash=false;if(dashRequested&&canUseExit({open:exitOpen,mode,paused,x:player.position.x,z:player.position.z,exit:arena.exit||EXIT})){useExit();return;}const dashMove=dashRequested&&dashLock<=0?spendDash(dashState):null;if(dashMove){runDashes++;invuln=Math.max(invuln,dashMove.invuln);player.userData.dashTime=dashMove.duration;player.userData.dashSpeed=dashMove.speed;player.userData.dashDir=lastMove.clone();vfx.burst(player.position,dashState.id==='molt'?'reflect':'seed',dashState.id==='molt'?22:14);vfx.dash(player.position,player.rotation.y);audio.play('dash');player.userData.dashFX=0;}if(player.userData.dashTime>0){player.userData.dashTime-=dt;player.position.addScaledVector(player.userData.dashDir,dt*(player.userData.dashSpeed||17));player.userData.dashFX+=dt;if(player.userData.dashFX>=.045){player.userData.dashFX=0;vfx.dash(player.position,player.rotation.y);}}else player.position.addScaledVector(move,dt*PLAYER_SPEED*runMoveScale(runBonuses)*(gardenStats()?.move||1)*seedTitle.state().moveSpeed*(playerSlow>0?.75:1)*(hasteTime>0?ITEMS.wind.speed:1));collide(player.position);aimRing.visible=false;targetTimer-=dt;if(targetTimer<=0||!cachedTarget||cachedTarget.dead){cachedTarget=acquireTarget(player.position,enemies,obstacles,segmentHitsCover);targetTimer=.1;}const autoTarget=cachedTarget;if(autoTarget)aim.copy(autoTarget.g.position);let aimdir=aim.clone().sub(player.position).setY(0).normalize();player.rotation.y=Math.atan2(aimdir.x,aimdir.z);playerMotion.update(dt,player.position.x-previousPlayer.x,player.position.z-previousPlayer.z,{dashing:player.userData.dashTime>0});player.userData.updateArt(camera,player.position.x-previousPlayer.x,player.position.z-previousPlayer.z);player.visible=invuln<.5||Math.floor(time*20)%2===0;player.userData.halo.rotation.z=time*.4;
if(autoTarget&&shootCD<=0&&!roomCleared){shootCD=.22/gardenCooldown();growth.fire();vfx.muzzle(player.position.clone().addScaledVector(aimdir,.65),aimdir,[...chosen][0]||'seed');projectile(player.position.clone().addScaledVector(aimdir,.6),aimdir);audio.play('shot');}
for(const [id,combat] of formCombats){const cd=(formCooldowns.get(id)||0)-dt;if(autoTarget&&cd<=0&&!roomCleared&&!FORMS[formAttacks.get(id)||id]?.passive){formCooldowns.set(id,combat.fire(player.position,aimdir,autoTarget.g.position)/gardenCooldown());audio.play('shot');}else formCooldowns.set(id,Math.max(-1,cd));}
