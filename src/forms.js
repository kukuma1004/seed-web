...
export const CURATED_FORMS=Object.freeze(Object.fromEntries([
 ...
 form('frostguard',['orbit','frost'],'서리 위성','서리 위성이 적을 베어 얼리고 일반 적 탄환을 막으며, 주기적으로 주변을 얼리는 냉기를 터뜨립니다. 보스 탄은 통과합니다.','근접 제압과 탄막 방어를 함께','사거리가 짧아 멀리 있는 적은 직접 쫓아가야 함',true),
 ...
 form('mirrorguard',['orbit','reflect'],'거울 수호','주위를 도는 거울이 일반 적 탄환을 되받습니다. 오스틴의 탄은 한 번에 하나씩 힘을 모아 돌려보내며, 다른 보스 탄은 통과합니다.','사수·포탑의 탄막을 공격으로 바꿈','탄을 쏘지 않는 근접 무리에게는 약함',true),
 ...
 form('comethalo',['orbit','burst'],'혜성 화관','꽃봉오리가 공전하며 일반 적 탄환을 막습니다. 씨앗이 움직인 거리로 화관을 충전하면 가까운 적에게 폭발 혜성을 쏩니다. 보스 탄은 통과합니다.','계속 이동하며 여러 폭발 경로를 이어 감','멈춰 있으면 충전이 빠져 혜성이 나오지 않음',true),
 form('stormanchor',['chain','gravity'],'뇌우 닻','번개가 세 적 이상을 이으면 중력 닻을 박습니다. 혼자 남은 오스틴에게는 두 차례 번개를 맞혀 닻을 충전합니다.','여러 방향의 적을 한 점으로 모아 후속 공격 준비','보스 충전 중에는 닻이 터지지 않아 연속 적중이 필요'),
 ...
 form('halobloom',['orbit','split'],'꽃잎 후광','씨앗을 도는 꽃잎 고리가 일반 적 탄환을 막고 적을 벱니다. 여덟 번 베면 꽃잎이 모두 바깥으로 만개합니다. 보스 탄은 통과합니다.','사방에서 붙는 무리를 베다가 한꺼번에 흩뿌림','만개한 뒤 고리가 잠시 비고 멀리 있는 적에게는 닿지 않음',true),
 form('frostnet',['chain','frost'],'얼어붙은 그물','번개가 세 적 이상을 이으면 서리 줄이 남습니다. 혼자 남은 오스틴에게는 짧은 서리 줄을 걸어 지나갈 때 상처를 냅니다.','길목을 얼려 무리의 발을 묶음','문지기·보스는 서리 줄에 닿아도 멈추지 않음'),
 form('rewindbolt',['chain','recall'],'되감는 번개','번개가 지나간 길을 기억했다가, 씨앗이 충분히 움직이면 그 길을 되감아 한 번 더 흐릅니다. 오스틴과의 단독 경로도 기억합니다.','움직이며 같은 무리를 두 번 훑음','제자리에 서 있으면 기억한 길이 그대로 사라짐'),
 ...
 form('pierceshower',['pierce','split'],'꿰뚫는 꽃비','창이 적을 꿰뚫을 때마다 꽃잎 두 장이 양옆으로 튑니다. 오스틴을 맞힌 꽃잎은 되돌아와 다시 찌릅니다.','줄지어 오는 무리를 창과 꽃잎으로 한 번에 훑음','일반 적이 혼자 있으면 꽃잎이 헛나감'),
 form('ebbring',['orbit','recall'],'밀물 고리','고리가 지나온 자리에 남았다가 밀물처럼 따라붙으며 적을 벱니다. 고리에 닿는 일반 적 탄환은 막지만 보스 탄은 통과합니다.','계속 움직이면 뒤쫓아 오는 적을 고리가 대신 베어 줌','멈춰 서면 고리가 작게 오므라들어 약해짐',true),
 ...
 form('spearring',['orbit','pierce'],'창날 고리','씨앗을 도는 창이 일반 적 탄환을 막습니다. 창 하나가 틈틈이 떨어져 나가 가장 가까운 적의 줄을 꿰뚫고 다시 자랍니다. 빈자리와 보스 탄은 막지 못합니다.','곁을 지키면서 방패병과 줄지어 선 적을 꿰뚫음','창이 빠진 동안 고리가 비고, 창을 쏘는 간격이 김',true),
 form('accretiondisk',['orbit','gravity'],'강착 원반','씨앗을 도는 소용돌이가 일반 적 탄환과 부스러기를 끌어모읍니다. 가득 차면 적들에게 내던지고, 보스 탄은 통과합니다.','탄막과 근접 무리를 모두 탄으로 바꿔 되돌려 줌','가득 찰 때까지는 근접 베기뿐이라 멀리 있는 적에게 약함',true),
...
const SOLO_ALL=Object.freeze(Object.fromEntries([
 ...
 solo('starring','orbit','별의 고리','넓어졌다 좁아지는 꽃잎 고리가 적을 베고 일반 적 탄환을 막습니다. 보스 탄은 통과합니다.','가까이 오는 적과 탄막을 동시에 막음','멀리서 버티는 적은 직접 다가가야 함',true),
