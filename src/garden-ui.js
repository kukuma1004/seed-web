...
import {harvestLine,SEEDS,SEED_IDS,GUARDIAN,PLOTS,ACTIVE_SLOTS,FRAGMENTS_PER_SEED,STAGES,STAGE_NAMES,STAGE_POINTS,
 stageOf,nextStagePoints,plantName,branchSummary,activePlants,gardenEffects,
...
export function renderGarden(root,{garden,onChange,onClose,picking=null}){
 const effects=gardenEffects(garden),actives=activePlants(garden);
 ...
 const plots=garden.plots.map((plant,index)=>{
  ...
  return `<div class="plot ${plant.active?'active':''} stage-${stage}">
   ...
   <div class="plot-actions">
    ${plant.branch&&grown?`<button class="primary small" data-active="${index}">${plant.active?'정원에서 빼기':'다음 여정에 데려가기'}</button>`:''}
 ...
 root.innerHTML=`<div class="garden-screen">
  ...
  <p class="garden-note">다음 여정에 데려갈 수 있는 식물은 ${ACTIVE_SLOTS}칸입니다. 지금 ${actives.length}칸을 쓰고 있어요.
 ...
 const update=(next,pick=null)=>{onChange(next);renderGarden(root,{garden:next,onChange,onClose,picking:pick});};
 root.querySelector('#garden-close').onclick=onClose;
 for(const button of root.querySelectorAll('[data-plant-here]'))
  button.onclick=()=>{if(Object.keys(garden.seeds).length)renderGarden(root,{garden,onChange,onClose,picking:Number(button.dataset.plantHere)});};
 ...
 for(const button of root.querySelectorAll('[data-active]')){
  ...
  button.onclick=()=>{const r=setActive(garden,index,!garden.plots[index].active);if(r.ok)update(r.garden);};
 }
 ...
 for(const button of root.querySelectorAll('[data-cancel]'))button.onclick=()=>renderGarden(root,{garden,onChange,onClose,picking:null});
 return root;
}
export {harvestLine};
