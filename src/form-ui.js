...
export function discoveryBook(profile){
 ...
 <div class="form-cards book">${Object.values(FORMS).map(f=>{const known=profile.forms.includes(f.id);return `<article class="form-card book-entry ${known?'':'unknown'}" title="${known?f.desc:'아직 발견하지 못한 진화'}">${formArt(f.id,'form-portrait')}<strong>${known?f.name:'？？？'}</strong><p>${f.pair}</p></article>`;}).join('')}</div>
