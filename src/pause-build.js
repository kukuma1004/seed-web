 ...
 const api={
  show(levels,forms){
   ...
   if(titles&&(titles.titles.length||titles.next)){
    ...
    // 도감 칭호는 다섯 능력을 한꺼번에 올린다.
    const sources=titles.codexBonus>0?`도감 칭호 · 모든 능력 +${percent(titles.codexBonus)}`:'';
    root.querySelector('#pause-loadout').insertAdjacentHTML('beforeend',`<section class="item-bag title-perk"><h3>칭호와 현재 능력치</h3><p class="title-total"><strong>${titles.codexBonus>0?`모든 능력 +${percent(titles.codexBonus)}`:'칭호 효과 없음'}</strong>${sources?`<small>${sources}</small>`:''}</p>${titles.maxHpBonus?`<p class="title-total"><strong>최대 생명력 +${titles.maxHpBonus}</strong><small>항상 적용되는 칭호 효과</small></p>`:''}${cadence>1?`<p class="title-total"><strong>공격 빈도 ${percent(cadence)}</strong><small>이번 여정 희귀 보너스 +${percent(cadence-1)}</small></p>`:''}${titles.titles.map(t=>`<p class="${t.id===titles.equipped?'equipped':''}"><strong>${t.id===titles.equipped?'장착 · ':''}${t.name}</strong> · ${t.perk}</p>`).join('')}${titles.next?`<p class="title-next">도감 ${titles.next.need}개 더 · ${titles.next.reward}</p>`:''}</section>`);
   }
